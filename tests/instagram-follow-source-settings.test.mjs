...
test("follow source rotation defaults use production package values", () => {
  assert.equal(FOLLOW_SOURCE_ROTATION_DEFAULTS.max_follows_per_target_per_run, 27);
  assert.equal(FOLLOW_SOURCE_ROTATION_DEFAULTS.max_targets_per_run, 4);
  assert.deepEqual(followSourceRotationDefaultsForPackage("Growth"), {
    max_follows_per_target_per_run: 27,
    max_targets_per_run: 4,
  });
  assert.deepEqual(followSourceRotationDefaultsForPackage("Pro"), {
    max_follows_per_target_per_run: 30,
    max_targets_per_run: 4,
  });
  assert.deepEqual(followSourceRotationDefaultsForPackage("Premium"), {
    max_follows_per_target_per_run: 30,
    max_targets_per_run: 4,
  });
  assert.deepEqual(followSourceRotationDefaultsForPackage("Package pending"), {
    max_follows_per_target_per_run: 27,
    max_targets_per_run: 4,
  });
});
