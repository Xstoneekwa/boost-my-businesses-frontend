...
Outbox preview (relay/admin): `GET /api/instagram-dashboard/email-lifecycle/outbox-preview` — read-only combined planner output from `buildClientEmailLifecycleOutboxPlan()` with lifecycle precedence (one effective candidate per account); no writes, sends, or intents.
