...
import { CT_REVIEW_COPY, projectCtReviewView, type CtReviewLanguage } from "@/lib/ct-premium/review-view-model";
        ...
        {scopedProposals.map((proposal) => {
          ...
          return (
            <article className={styles.card} data-testid={`ct-proposal-card-${proposal.id}`} key={proposal.id}>
              <header><div><h3>@{proposal.normalizedUsername}</h3><p>{proposal.displayName}</p></div><span className={styles.score}>{proposal.score.total}/100</span></header>
