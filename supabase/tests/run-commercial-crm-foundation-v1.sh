...
TEST_ROOT="$(mktemp -d /private/tmp/commercial-crm-foundation-v1.XXXXXX)"
PGDATA="${TEST_ROOT}/pgdata"
PGPORT="${COMMERCIAL_CRM_FOUNDATION_TEST_PGPORT:-55481}"
...
"${PG_BIN}/pg_ctl" -D "${PGDATA}" \
  ...
  -w start >/dev/null
"${PG_BIN}/createdb" -p "${PGPORT}" -U postgres commercial_crm_foundation_v1
"${PG_BIN}/psql" -X -v ON_ERROR_STOP=1 -p "${PGPORT}" -U postgres \
  -d commercial_crm_foundation_v1 \
  -f supabase/tests/commercial-crm-foundation-v1.sql
