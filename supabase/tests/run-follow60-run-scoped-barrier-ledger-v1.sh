...
TEST_ROOT="$(mktemp -d /private/tmp/follow60-ledger-v1.XXXXXX)"
PGDATA="${TEST_ROOT}/pgdata"
PGPORT="${FOLLOW60_LEDGER_TEST_PGPORT:-55447}"
...
"${PG_BIN}/createdb" -p "${PGPORT}" -U postgres follow60_ledger_v1
"${PG_BIN}/psql" -X -v ON_ERROR_STOP=1 -p "${PGPORT}" -U postgres \
  -d follow60_ledger_v1 -f supabase/tests/follow60-run-scoped-barrier-ledger-v1.sql
