...
TEST_ROOT="$(mktemp -d /private/tmp/login-state-monotonic-v1.XXXXXX)"
PGDATA="${TEST_ROOT}/pgdata"
PGPORT="${LOGIN_STATE_MONOTONIC_TEST_PGPORT:-55472}"
...
"${PG_BIN}/createdb" -p "${PGPORT}" -U postgres login_state_monotonic_v1
"${PG_BIN}/psql" -X -v ON_ERROR_STOP=1 -p "${PGPORT}" -U postgres \
  -d login_state_monotonic_v1 \
  -f supabase/tests/login-state-monotonic-v1.sql
