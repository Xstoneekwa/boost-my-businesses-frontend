-- FAST-TRACK FIXTURE ONLY — isolated test DB nxntngkhkoynljcagmkq.
-- Minimal public baseline for Plan Change migration/RPC validation.
-- NOT source parity. Does NOT recreate auth.users. No generic DROP SCHEMA/TABLE.
...
-- FAST-TRACK FIXTURE ONLY — helper for quote source_revision (not source DB parity).
create or replace function public.plan_change_test_source_revision(
...
comment on function public.plan_change_test_source_revision is
  'FAST-TRACK test fixture helper only — not a reproduction of production trigger behavior';
