...
begin
  ...
  select public.transition_account_dashboard_action(
    p_action_id,
    'resolved',
    'admin',
    p_actor_id,
  ...
  insert into public.ig_action_logs (
    ...
    null,
    null,
    'dashboard_action_reviewed',
    'success',
    'operator_review_action_resolved',
    jsonb_build_object(
      'actor_type', 'admin',
      'actor_id', p_actor_id,
      'source', v_source,
...
revoke all on function public.review_operator_dashboard_action(uuid, uuid, uuid, text, text, jsonb) from anon;
revoke all on function public.review_operator_dashboard_action(uuid, uuid, uuid, text, text, jsonb) from authenticated;
grant execute on function public.review_operator_dashboard_action(uuid, uuid, uuid, text, text, jsonb) to service_role;

comment on function public.review_operator_dashboard_action(uuid, uuid, uuid, text, text, jsonb) is
  'Atomically resolves one operator_review_required action, clears its campaign block, and records the authenticated operator audit without changing the linked incident.';
