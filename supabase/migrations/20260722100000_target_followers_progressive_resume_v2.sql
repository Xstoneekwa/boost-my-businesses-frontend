...
-- Persistent, server-only progress checkpoints for one account + target Followers list.
-- No historical depth is backfilled by this migration.
...
create table if not exists public.ig_target_followers_resume_checkpoints (
  ...
  constraint ig_target_followers_resume_checkpoint_lease_mode_check
    check (lease_mode is null or lease_mode in ('shadow', 'enforce')),
...
create table if not exists public.ig_target_followers_resume_checkpoint_events (
  ...
  mode text null,
  previous_optimistic_version bigint null,
  new_optimistic_version bigint null,
  previous_depth integer null,
  new_depth integer null,
...
revoke all on table public.ig_target_followers_resume_checkpoints from public, anon, authenticated;
revoke all on table public.ig_target_followers_resume_checkpoint_events from public, anon, authenticated;
grant select, insert, update on table public.ig_target_followers_resume_checkpoints to service_role;
grant select, insert on table public.ig_target_followers_resume_checkpoint_events to service_role;
...
begin
  ...
  update public.ig_target_followers_resume_checkpoints c
  set status = p_status, invalidation_reason = p_reason,
      lease_owner_run_id = null, lease_mode = null, lease_expires_at = null,
      last_run_id = p_run_id, optimistic_version = c.optimistic_version + 1, updated_at = now()
  ...
  insert into public.ig_target_followers_resume_checkpoint_events (
    ...
    v_row.last_safe_depth, v_row.last_safe_depth, p_reason
  );
...
begin
  ...
  insert into public.ig_target_followers_resume_checkpoint_events (
    ...
    greatest(case when p_reset_enforce then 0 else v_row.last_safe_depth end, case when p_reset_shadow then 0 else v_row.shadow_last_safe_depth end),
    p_reason,
...
revoke execute on function public.get_target_followers_resume_checkpoint(uuid, uuid, text) from public, anon, authenticated;
revoke execute on function public.claim_target_followers_resume_checkpoint(uuid, uuid, text, text, uuid, text, bigint, integer) from public, anon, authenticated;
revoke execute on function public.commit_target_followers_resume_checkpoint(uuid, uuid, text, uuid, text, bigint, integer, text, text, jsonb, text, text, text) from public, anon, authenticated;
revoke execute on function public.invalidate_target_followers_resume_checkpoint(uuid, uuid, text, uuid, bigint, text, text) from public, anon, authenticated;
revoke execute on function public.reset_target_followers_resume_checkpoint(uuid, uuid, text, uuid, bigint, text, boolean, boolean) from public, anon, authenticated;
