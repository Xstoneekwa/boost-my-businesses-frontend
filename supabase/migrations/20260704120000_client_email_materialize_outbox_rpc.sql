-- Local migration only. Do not apply without explicit GO (TASK 13A / hardened TASK 13B).
-- Transactional materialize RPC with strict parent/category rules, idempotency identity checks,
-- and account/client ownership verification via public.client_instagram_accounts.
-- No trigger, cron, queue, webhook, or provider call.
...
begin
  ...
  if p_category = 'needs_more_target_accounts' then
    if p_operation = 'create_needs_more_reminder_intent' then
      if p_parent_id is null then
        return jsonb_build_object('ok', false, 'code', 'needs_more_active_sequence_required');
      end if;
  ...
  if p_recipient_email is null or btrim(p_recipient_email) = '' then
    return jsonb_build_object('ok', false, 'code', 'missing_recipient_email');
  end if;
  ...
  if p_trigger is null
     ...
     or p_from_email is null
     or p_from_email_snapshot is null
     or p_support_email_snapshot is null then
    return jsonb_build_object('ok', false, 'code', 'missing_intent_snapshot_fields');
  end if;
  ...
  if v_intent_id is null then
    ...
    if not found then
      return jsonb_build_object('ok', false, 'code', 'intent_create_failed');
    end if;
...
comment on function public.materialize_client_email_outbox_candidate_v1(
  ...
  'Server-only transactional materialize for client email outbox. Validates account/client ownership, strict parent/category operations, and idempotency business identity before returning existing intents without snapshot rewrite.';
