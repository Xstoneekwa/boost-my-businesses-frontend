-- Client email materialize RPC: from_email / from_email_snapshot consistency (TASK 13E).
-- CREATE OR REPLACE only — rejects mismatched or missing sender params before parent/intent writes.
...
comment on function public.materialize_client_email_outbox_candidate_v1(
  ...
  'Server-only transactional materialize for client email outbox. Validates account/client ownership, from_email/from_email_snapshot consistency, strict parent/category operations, and idempotency business identity before returning existing intents without snapshot rewrite.';
