-- Follow 60s mid-canary: durable stage receipts and an operator evaluation
-- barrier. Additive and account scoped. No scheduler/run is started here.

alter table public.ig_interaction_events
  add column if not exists stage_idempotency_key text;

create unique index if not exists ig_interaction_events_stage_idempotency_uidx
  on public.ig_interaction_events (account_id, run_id, stage_idempotency_key)
  where stage_idempotency_key is not null;

create table if not exists public.follow_60s_canary_controls (
  ...
  request_id uuid references public.account_run_requests(id) on delete set null,
  barrier_reached_at timestamptz,
  hold_armed_at timestamptz,
  released_at timestamptz,
  metadata_safe jsonb not null default '{}'::jsonb,
  updated_at timestamptz not null default now(),
  constraint follow_60s_canary_controls_status_check check (
    status in ('disabled','armed','barrier_waiting_stop','waiting_operator_evaluation','continuation_authorized','completed')
  ),
  ...
  constraint follow_60s_canary_controls_metadata_check check (jsonb_typeof(metadata_safe) = 'object')
);
...
as $$
  select case when auth.role() = 'service_role' then
    coalesce((select to_jsonb(c) from public.follow_60s_canary_controls c where c.account_id = p_account_id), '{}'::jsonb)
  else '{}'::jsonb end
$$;
...
begin
  ...
  if p_account_id <> 'ba73eda4-d22a-4b93-9683-2af7b8aab764'::uuid
...
create or replace function public.mark_follow_60s_canary_evaluation_hold_v1(
  p_account_id uuid, p_run_id uuid, p_request_id uuid, p_metadata_safe jsonb default '{}'::jsonb
)
returns jsonb
language plpgsql
security definer
...
begin
  if auth.role() <> 'service_role' then raise exception 'service_role_required' using errcode='42501'; end if;
  update public.follow_60s_canary_controls set status='waiting_operator_evaluation', run_id=p_run_id,
    request_id=p_request_id, hold_armed_at=now(), updated_at=now(),
    metadata_safe=metadata_safe || coalesce(p_metadata_safe,'{}'::jsonb)
    where account_id=p_account_id and status in ('armed','barrier_waiting_stop','waiting_operator_evaluation');
  if not found then raise exception 'follow_60s_hold_not_eligible' using errcode='55000'; end if;
  return jsonb_build_object('ok',true,'status','waiting_operator_evaluation');
end;
$$;

revoke all on function public.get_follow_60s_canary_control_v1(uuid) from public, anon, authenticated;
revoke all on function public.persist_follow_60s_stage_v1(uuid,uuid,uuid,text,text,text,text,text,timestamptz,jsonb) from public, anon, authenticated;
revoke all on function public.mark_follow_60s_canary_barrier_v1(uuid,uuid,uuid,integer) from public, anon, authenticated;
revoke all on function public.mark_follow_60s_canary_evaluation_hold_v1(uuid,uuid,uuid,jsonb) from public, anon, authenticated;
grant execute on function public.get_follow_60s_canary_control_v1(uuid) to service_role;
grant execute on function public.persist_follow_60s_stage_v1(uuid,uuid,uuid,text,text,text,text,text,timestamptz,jsonb) to service_role;
grant execute on function public.mark_follow_60s_canary_barrier_v1(uuid,uuid,uuid,integer) to service_role;
grant execute on function public.mark_follow_60s_canary_evaluation_hold_v1(uuid,uuid,uuid,jsonb) to service_role;
