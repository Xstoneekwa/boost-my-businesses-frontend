  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (signInError || !data.session) {
        throw new Error(signInError?.message ?? t.login.sessionError);
      }
      ...
      const sessionPayload = (await sessionResponse.json()) as { error?: string; details?: string };

      if (!sessionResponse.ok) {
        throw new Error(sessionPayload.details || sessionPayload.error || t.login.accessError);
      }
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "32px 24px 72px" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18, marginBottom: 58, flexWrap: "wrap" }}>
          <Link href="/agent/restaurant-call-assistant" style={{ display: "inline-flex", alignItems: "center", gap: 10, color: "#f0f0ef", textDecoration: "none" }}>
            ...
            <span style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.42)", fontSize: 11 }}>{t.login.brandSub}</span>
            </span>
          </Link>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 390px), 1fr))", gap: 28, alignItems: "center" }}>
          <div>
            ...
            <p style={{ color: "rgba(255,255,255,0.58)", fontSize: 16, lineHeight: 1.75, maxWidth: 610, marginBottom: 26 }}>
              {t.login.text}
            </p>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {t.login.badges.map((item) => (
                <span
                  key={item}
                  style={{
          ...
          >
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 9 }}>
                <Link href="/restaurant-forgot-password" style={{ color: AC_TEXT, fontSize: 12.5, fontWeight: 800, textDecoration: "none" }}>
            ...
            <button
              ...
              style={{
                ...
                background: isSubmitting ? "rgba(255,255,255,0.08)" : AC,
                color: isSubmitting ? "rgba(255,255,255,0.44)" : "#160b02",
                borderRadius: 999,
                fontSize: 14,
                fontWeight: 900,
                cursor: isSubmitting ? "wait" : "pointer",
                boxShadow: isSubmitting ? "none" : "0 8px 32px rgba(245,158,11,0.28)",
                marginBottom: 14,
              }}
            >
