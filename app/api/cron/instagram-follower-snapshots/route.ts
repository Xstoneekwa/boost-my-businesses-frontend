...
export async function GET(request: Request) {
  ...
  if (dryRun) {
    try {
      ...
      return NextResponse.json({ ok: false, error: "follower_snapshot_dry_run_failed" }, { status: 500 });
    }
  }
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: "follower_snapshot_cron_failed" }, { status: 500 });
  }
}
