...
function Dashboard({ model, reviewQueue, reviewPage }: { model: CommercialDashboardReadModel; reviewQueue: CommercialReviewQueue; reviewPage: number }) {
  const kpiCards = [
...
export default async function CommercialDashboardPage({ searchParams }: { searchParams?: Promise<CommercialDashboardSearchParams> }) {
  ...
  try {
    ...
    [model, reviewQueue] = await Promise.all([
  ...
  return <Dashboard model={model} reviewQueue={reviewQueue} reviewPage={reviewPage} />;
}
