  ...
  return <main className="commercial-detail-page">
    ...
    <section className="commercial-detail-columns">
      ...
      <AnalyticsSectionCard eyebrow="Qualification" title="Review state">
        <DetailGrid items={[["Lead score", lead.qualification.leadScore?.toFixed(1) ?? "—"], ["Score priority", lead.qualification.scorePriority], ["CRM score", lead.qualification.score?.toString() ?? "—"], ["Priority", pretty(lead.qualification.priority)], ["Status", pretty(lead.qualification.status)], ["Needs Liam review", lead.qualification.needsManualReview ? "Yes" : "No"], ["AI confidence", lead.qualification.aiConfidence === null ? "—" : `${Math.round(lead.qualification.aiConfidence * 100)}%`], ["Scoring model", lead.qualification.scoringModelVersion], ["AI model", lead.qualification.aiModel], ["Prompt", lead.qualification.aiPromptVersion], ["Scored at", date(lead.qualification.scoredAt)], ["Hard gates", lead.qualification.hardGateCodes.join(", ") || "None"], ["Approved by", lead.qualification.approvedBy], ["Approved at", date(lead.qualification.approvedAt)]]} />
        <div className="commercial-context-columns"><div><h3>Score breakdown</h3><SafeContext value={lead.qualification.scoreBreakdown} /></div><div><h3>Enrichment provenance</h3><SafeContext value={lead.business.provenance} /></div></div>
    ...
    <section className="commercial-conversion"><AnalyticsSectionCard eyebrow="Discovery + enrichment" title="Observed business intelligence">
      <DetailGrid items={[["Description", lead.business.description], ["Business status", pretty(lead.business.status)], ["Booking URL", lead.business.bookingUrl ? <a href={lead.business.bookingUrl} target="_blank" rel="noreferrer">{lead.business.bookingUrl}</a> : "—"], ["Last enriched", date(lead.business.lastEnrichedAt)]]} />
      <div className="commercial-context-columns"><div><h3>Public Instagram snapshot</h3><SafeContext value={lead.business.enrichment} /></div><div><h3>Real audience suggestions</h3><SafeContext value={lead.qualification.audienceContext} /></div></div>
