...
test("direct client account creation is closed while account subroutes remain intact", () => {
  ...
  assert.match(accountsSection, /renderPanel = true/);
  assert.match(accountsSection, /\{renderPanel \? <section className="cd-card cd-accounts-panel">/);
  assert.match(dashboard, /renderPanel=\{!agencyModeActive\}/);
});
...
test("wizard implements exactly the five required steps and keeps Auto Login explicit", () => {
  const wizard = source("../../app/instagram-client/ClientInstagramOnboardingWizard.tsx");
  ...
  assert.match(onboardingService, /login_status: "pending_login"/);
