...
import {
  ...
  PROFILE_INTELLIGENCE_LEGACY_PROMPT_VERSION_FR,
  ...
  validateProfileAiTargetingQuality,
...
import { confirmedProfileTargetingDraft } from "./profile-intelligence-ui.ts";
...
function output(overrides = {}, outputLanguage = "fr") {
  return {
    ...
    suggestions: {
      suggested_category: { value: "Automatisation et marketing Instagram", confidence: "high", evidence_fields: ["biography"] },
      niche: { value: "Automatisation Instagram pour la croissance des entreprises", confidence: "high", evidence_fields: ["biography", "caption_samples"] },
      probable_audience: { value: "Entrepreneurs, indépendants, petites entreprises, agences et créateurs souhaitant automatiser leur présence et leur croissance sur Instagram.", confidence: "medium", evidence_fields: ["biography"] },
      themes: { value: ["automatisation Instagram", "stratégie de croissance", "acquisition client", "performance marketing", "développement commercial"], confidence: "high", evidence_fields: ["biography", "caption_samples"] },
      business_description: { value: "Services d'automatisation et de stratégie Instagram destinés à développer la visibilité, l'acquisition et les performances des entreprises.", confidence: "medium", evidence_fields: ["biography"] },
      keywords: { value: ["automatisation Instagram", "croissance Instagram", "acquisition client", "marketing digital", "stratégie Instagram", "développement commercial"], confidence: "high", evidence_fields: ["biography"] },
...
function englishOutput() {
  return output({
    suggested_category: { value: "Instagram Marketing Automation Services", confidence: "high", evidence_fields: ["biography"] },
    niche: { value: "Instagram automation for small business growth", confidence: "high", evidence_fields: ["biography", "caption_samples"] },
    probable_audience: { value: "Small business owners, agencies, creators, and online entrepreneurs seeking to automate and grow their Instagram presence.", confidence: "medium", evidence_fields: ["biography"] },
    ...
    keywords: { value: ["Instagram automation", "business growth", "content strategy", "digital marketing", "lead generation", "customer acquisition"], confidence: "high", evidence_fields: ["biography"] },
...
test("structured output schema is strict, complete and versioned", () => {
  assert.equal(PROFILE_INTELLIGENCE_PROMPT_VERSION, "profile_intelligence_v2_prompt_v5_targeting_ready_fr");
  assert.equal(PROFILE_INTELLIGENCE_PROMPT_VERSION_FR, "profile_intelligence_v2_prompt_v5_targeting_ready_fr");
  assert.equal(PROFILE_INTELLIGENCE_PROMPT_VERSION_EN, "profile_intelligence_v2_prompt_v5_targeting_ready_en");
  ...
  assert.equal(validateProfileAiStructuredOutput(output())?.themes.value.length, 5);
});
...
test("localized V5 targeting-ready no-geo prompts are selected deterministically", () => {
  ...
  assert.match(buildProfileIntelligenceUserPrompt(snapshot()), /prompt_version=profile_intelligence_v2_prompt_v5_targeting_ready_fr/);
  assert.match(buildProfileIntelligenceUserPrompt(englishSnapshot), /prompt_version=profile_intelligence_v2_prompt_v5_targeting_ready_en/);
});
...
test("default gpt-4o-mini call uses the exact minimal Responses allowlist", async () => {
  ...
  assert.equal(result.noGeoValid, true);
  assert.equal(result.targetingQualityValid, true);
  assert.equal(result.targetingQualityValidation.valid, true);
  assert.deepEqual(result.targetingQualityValidation.reasons, []);
  assert.equal(result.fieldQuality.exclusions.status, "empty_valid");
...
test("explicit gpt-4o-mini benchmark call keeps Responses allowlist and benchmark pricing", async () => {
  ...
  assert.match(body.input[1].content[0].text, /prompt_version=profile_intelligence_v2_prompt_v5_targeting_ready_en/);
  assert.equal(result.metrics.estimated_cost_usd, 0.00024);
});

test("targeting-ready validator accepts precise natural French and English outputs", () => {
  const french = validateProfileAiStructuredOutput(output(), "fr");
  const english = validateProfileAiStructuredOutput(englishOutput(), "en");
  assert.equal(validateProfileAiTargetingQuality(french).valid, true);
  assert.equal(validateProfileAiTargetingQuality(english).valid, true);
  assert.deepEqual(validateProfileAiTargetingQuality(french).reasons, []);
});

test("field-level quality neutralizes a weak optional category without blocking valid core fields", () => {
  const weakCategory = validateProfileAiStructuredOutput(output({
    suggested_category: { value: "Automatisation", confidence: "high", evidence_fields: ["biography"] },
  }));
  const result = validateProfileAiTargetingQuality(weakCategory);
  assert.equal(result.valid, true);
  assert.deepEqual(result.fieldQuality.suggested_category, { status: "insufficient", reason: "suggested_category_too_generic" });
  assert.equal(result.suggestions.suggested_category.value, null);
  assert.equal(result.suggestions.niche.value, weakCategory.niche.value);
  assert.equal(JSON.stringify(result).includes('"value":"Automatisation"'), false);
});

test("nullable optional fields and empty exclusions are empty_valid", () => {
  const parsed = validateProfileAiStructuredOutput(output({
    suggested_category: { value: null, confidence: "low", evidence_fields: [] },
    business_description: { value: null, confidence: "low", evidence_fields: [] },
    exclusions: { value: [], confidence: "low", evidence_fields: [] },
  }));
  const result = validateProfileAiTargetingQuality(parsed);
  assert.equal(result.valid, true);
  assert.equal(result.fieldQuality.suggested_category.status, "empty_valid");
  assert.equal(result.fieldQuality.business_description.status, "empty_valid");
  assert.equal(result.fieldQuality.exclusions.status, "empty_valid");
});

test("weak optional description is neutralized without blocking valid core fields", () => {
  const parsed = validateProfileAiStructuredOutput(output({
    business_description: { value: "services d'automatisation", confidence: "medium", evidence_fields: ["biography"] },
  }));
  const result = validateProfileAiTargetingQuality(parsed);
  assert.equal(result.valid, true);
  assert.deepEqual(result.fieldQuality.business_description, { status: "insufficient", reason: "business_description_too_short" });
  assert.equal(result.suggestions.business_description.value, null);
});

test("essential niche and audience are neutralized and block global readiness", () => {
  const weakNiche = validateProfileAiStructuredOutput(output({
    niche: { value: "Automatisation Instagram", confidence: "high", evidence_fields: ["biography"] },
  }));
  const weakAudience = validateProfileAiStructuredOutput(output({
    probable_audience: { value: "Entrepreneurs et professionnels", confidence: "medium", evidence_fields: ["biography"] },
  }));
  const nicheResult = validateProfileAiTargetingQuality(weakNiche);
  const audienceResult = validateProfileAiTargetingQuality(weakAudience);
  assert.equal(nicheResult.valid, false);
  assert.equal(nicheResult.suggestions.niche.value, null);
  assert.deepEqual(nicheResult.fieldQuality.niche, { status: "insufficient", reason: "niche_too_generic" });
  assert.equal(audienceResult.valid, false);
  assert.equal(audienceResult.suggestions.probable_audience.value, null);
  assert.deepEqual(audienceResult.fieldQuality.probable_audience, { status: "insufficient", reason: "probable_audience_too_generic" });
});

test("global readiness requires at least three useful themes and four useful keywords", () => {
  const twoThemes = validateProfileAiStructuredOutput(output({
    themes: { value: ["acquisition client", "performance marketing", "Instagram"], confidence: "high", evidence_fields: ["biography"] },
  }));
  const threeKeywords = validateProfileAiStructuredOutput(output({
    keywords: { value: ["acquisition client", "marketing digital", "stratégie Instagram"], confidence: "high", evidence_fields: ["biography"] },
  }));
  const ready = validateProfileAiStructuredOutput(output({
    themes: { value: ["automatisation Instagram", "stratégie de croissance", "acquisition client", "performance marketing"], confidence: "high", evidence_fields: ["biography"] },
    keywords: { value: ["automatisation Instagram", "croissance Instagram", "acquisition client", "marketing digital", "stratégie Instagram"], confidence: "high", evidence_fields: ["biography"] },
  }));
  const themesResult = validateProfileAiTargetingQuality(twoThemes);
  const keywordsResult = validateProfileAiTargetingQuality(threeKeywords);
  assert.equal(themesResult.valid, false);
  assert.equal(themesResult.suggestions.themes.value.length, 2);
  assert.equal(themesResult.fieldQuality.themes.status, "insufficient");
  assert.equal(keywordsResult.valid, false);
  assert.equal(keywordsResult.suggestions.keywords.value.length, 3);
  assert.equal(keywordsResult.fieldQuality.keywords.status, "insufficient");
  assert.equal(validateProfileAiTargetingQuality(ready).valid, true);
});

test("generic list entries are selectively removed while useful entries survive", () => {
  const parsed = validateProfileAiStructuredOutput(output({
    themes: { value: ["Instagram", "automatisation Instagram", "stratégie de croissance", "acquisition client"], confidence: "high", evidence_fields: ["biography"] },
    keywords: { value: ["business", "automatisation Instagram", "croissance Instagram", "acquisition client", "marketing digital"], confidence: "high", evidence_fields: ["biography"] },
  }));
  const result = validateProfileAiTargetingQuality(parsed);
  assert.equal(result.valid, true);
  assert.deepEqual(result.suggestions.themes.value, ["automatisation Instagram", "stratégie de croissance", "acquisition client"]);
  assert.deepEqual(result.suggestions.keywords.value, ["automatisation Instagram", "croissance Instagram", "acquisition client", "marketing digital"]);
});

test("quality-insufficient provider output fails after one fetch and persists no suggestions", async () => {
  let calls = 0;
  const weak = output({
    suggested_category: { value: "Automatisation", confidence: "high", evidence_fields: ["biography"] },
    niche: { value: "Automatisation Instagram", confidence: "high", evidence_fields: ["biography"] },
    probable_audience: { value: "Entrepreneurs et professionnels", confidence: "medium", evidence_fields: ["biography"] },
    themes: { value: ["Instagram", "croissance", "stratégie", "performance"], confidence: "high", evidence_fields: ["biography"] },
    business_description: { value: "services d'automatisation", confidence: "medium", evidence_fields: ["biography"] },
    keywords: { value: ["Instagram", "business", "performance", "stratégie", "marketing"], confidence: "high", evidence_fields: ["biography"] },
  });
  const result = await callProfileIntelligenceOpenAi({
    snapshot: snapshot(),
    apiKey: "test-key",
    fetchImpl: async () => {
      calls += 1;
      return new Response(JSON.stringify({
        output: [{ type: "message", content: [{ type: "output_text", text: JSON.stringify(weak) }] }],
      }), { status: 200 });
    },
  });
  assert.equal(calls, 1);
  assert.equal(result.schemaValid, true);
  assert.equal(result.businessOutputValid, true);
  assert.equal(result.targetingQualityValid, false);
  assert.equal(result.errorCode, "output_targeting_quality_insufficient");
  assert.equal(result.suggestions, null);
  const service = readFileSync(new URL("./client-account-onboarding.ts", import.meta.url), "utf8");
  assert.match(service, /suggestions: providerResult\.ok \? providerResult\.suggestions : targetingQualityFailure \? null : previousAiAnalysis\.suggestions/);
  assert.match(service, /field_quality: providerResult\.fieldQuality/);
  assert.doesNotMatch(service, /console\.(?:info|log)\([^)]*(?:suggestions|rejected_value)/s);
});

test("weak optional category succeeds after one fetch and only that field is neutralized", async () => {
  let calls = 0;
  const result = await callProfileIntelligenceOpenAi({
    snapshot: snapshot(),
    apiKey: "test-key",
  ...
  assert.equal(result.targetingQualityValid, true);
  assert.equal(result.suggestions.suggested_category.value, null);
  assert.equal(result.suggestions.niche.value, "Automatisation Instagram pour la croissance des entreprises");
  assert.deepEqual(result.fieldQuality.suggested_category, { status: "insufficient", reason: "suggested_category_too_generic" });
});

test("neutralized optional text is not projected to the browser and remains manually editable", () => {
  const raw = validateProfileAiStructuredOutput(output({
    suggested_category: { value: "Automatisation", confidence: "high", evidence_fields: ["biography"] },
  }));
  const qualityResult = validateProfileAiTargetingQuality(raw);
  const stored = withProfileAiAnalysis(buildStoredPublicAnalysis(profile()), {
    ...emptyProfileAiAnalysis(),
    status: "completed",
    requested_at: "2026-07-21T18:00:00.000Z",
    completed_at: "2026-07-21T18:00:04.000Z",
    suggestions: qualityResult.suggestions,
    field_quality: qualityResult.fieldQuality,
    targeting_quality_valid: qualityResult.valid,
  });
  const projected = projectClientPublicAnalysis(stored);
  assert.equal(projected.suggestedCategory, null);
  assert.deepEqual(projected.aiAnalysis.fieldQuality.suggested_category, {
    status: "insufficient",
    reason: "suggested_category_too_generic",
  });
  assert.equal(JSON.stringify(projected).includes('"suggestedCategory":"Automatisation"'), false);

  const confirmed = applyClientPublicAnalysisConfirmation({
    ...projected,
    suggestedCategory: "Automatisation et marketing Instagram",
  }, stored, "2026-07-21T18:05:00.000Z");
  assert.equal(confirmed.ai_analysis.suggestions.suggested_category.value, null);
  assert.equal(confirmed.ai_analysis.confirmed_values.suggested_category, "Automatisation et marketing Instagram");
  assert.equal(projectClientPublicAnalysis(confirmed).sources.suggestedCategory, "user_confirmed");
});

test("field quality sanitizer retains only statuses and normalized reason codes", () => {
  const sanitized = readStoredProfileAiAnalysis({
    ...emptyProfileAiAnalysis(),
    field_quality: {
      suggested_category: { status: "insufficient", reason: "suggested_category_too_generic", rejected_text: "must-not-survive" },
      niche: { status: "valid", reason: null },
      probable_audience: { status: "valid", reason: null },
      themes: { status: "valid", reason: null },
      business_description: { status: "empty_valid", reason: null },
      keywords: { status: "valid", reason: null },
      exclusions: { status: "empty_valid", reason: null },
    },
    targeting_quality_valid: true,
  });
  assert.equal(sanitized.targeting_quality_valid, true);
  assert.deepEqual(sanitized.field_quality.suggested_category, {
    status: "insufficient",
    reason: "suggested_category_too_generic",
  });
...
test("V2 suggestions survive serialization, remain distinct, and become user confirmed", () => {
  ...
  assert.equal(projectClientPublicAnalysis(refreshed)?.niche, "Automatisation Instagram pour la croissance des entreprises");
  ...
  assert.equal(confirmed.ai_analysis?.suggestions?.niche.value, "Automatisation Instagram pour la croissance des entreprises");
...
test("confirmed V5 values project into the targeting draft without biography or themes aliases", () => {
  const factual = buildStoredPublicAnalysis(profile());
  const analyzed = withProfileAiAnalysis(factual, {
    ...emptyProfileAiAnalysis(),
    status: "completed",
    request_key: "key-targeting",
    requested_at: "2026-07-21T18:00:00.000Z",
    completed_at: "2026-07-21T18:00:03.000Z",
    suggestions: validateProfileAiStructuredOutput(output()),
  });
  const projected = projectClientPublicAnalysis(analyzed);
  const confirmed = applyClientPublicAnalysisConfirmation({
    ...projected,
    probableAudience: "Audience confirmée avec une intention commerciale précise.",
    niche: "Niche confirmée pour automatisation Instagram B2B",
    businessDescription: "Description confirmée distincte de la biographie publique.",
    themes: ["thème confirmé un", "thème confirmé deux", "thème confirmé trois"],
    keywords: ["mot-clé confirmé alpha", "mot-clé confirmé beta", "mot-clé confirmé gamma", "mot-clé confirmé delta"],
  }, analyzed, "2026-07-21T18:05:00.000Z");
  const confirmedProjection = projectClientPublicAnalysis(confirmed);
  const draft = confirmedProfileTargetingDraft(confirmedProjection);
  assert.equal(confirmed.ai_analysis?.confirmed_values?.business_description, "Description confirmée distincte de la biographie publique.");
  assert.deepEqual(confirmed.ai_analysis?.confirmed_values?.keywords, ["mot-clé confirmé alpha", "mot-clé confirmé beta", "mot-clé confirmé gamma", "mot-clé confirmé delta"]);
  assert.equal(draft.idealCustomer, "Audience confirmée avec une intention commerciale précise.");
  assert.equal(draft.niche, "Niche confirmée pour automatisation Instagram B2B");
  assert.equal(draft.businessDescription, "Description confirmée distincte de la biographie publique.");
  assert.notEqual(draft.businessDescription, profile().biography);
  assert.deepEqual(draft.themes, ["thème confirmé un", "thème confirmé deux", "thème confirmé trois"]);
  assert.deepEqual(draft.keywords, ["mot-clé confirmé alpha", "mot-clé confirmé beta", "mot-clé confirmé gamma", "mot-clé confirmé delta"]);
...
test("AI analysis implementation cannot create business records or call Target AI/SearchAPI", () => {
  ...
  assert.match(section, /prompt_version: profileIntelligencePromptVersion\(outputLanguage\)/);
  assert.match(section, /targeting_quality_valid: providerResult\.businessOutputValid/);
});
...
test("stored AI sanitizer preserves only the bounded contract", () => {
  const sanitized = readStoredProfileAiAnalysis({
    ...
    confirmed_values: { niche: "Automatisation Instagram pour la croissance des entreprises", suggested_activity_area: "Johannesburg" },
    secret: "must-not-survive",
  });
  assert.equal(sanitized.status, "completed");
  assert.equal(sanitized.suggestions?.niche.value, "Automatisation Instagram pour la croissance des entreprises");
...
test("stored V4 analysis remains visible and keeps its legacy prompt provenance", () => {
  const legacy = readStoredProfileAiAnalysis({
    ...emptyProfileAiAnalysis(),
    status: "completed",
    prompt_version: PROFILE_INTELLIGENCE_LEGACY_PROMPT_VERSION_FR,
    completed_at: "2026-07-21T20:00:00.000Z",
    suggestions: {
      suggested_category: { value: "automatisation marketing", confidence: "medium", evidence_fields: ["biography"] },
      niche: { value: "automatisation Instagram", confidence: "high", evidence_fields: ["biography"] },
      probable_audience: { value: "entrepreneurs et professionnels", confidence: "medium", evidence_fields: ["biography"] },
      themes: { value: ["automatisation", "Instagram", "croissance", "stratégie", "performance"], confidence: "medium", evidence_fields: ["biography"] },
      business_description: { value: "services d'automatisation", confidence: "medium", evidence_fields: ["biography"] },
      keywords: { value: ["Instagram", "automatisation"], confidence: "medium", evidence_fields: ["biography"] },
      exclusions: { value: [], confidence: "low", evidence_fields: [] },
    },
  });
  assert.equal(legacy.prompt_version, PROFILE_INTELLIGENCE_LEGACY_PROMPT_VERSION_FR);
  assert.equal(legacy.status, "completed");
  assert.equal(legacy.suggestions?.suggested_category.value, "automatisation marketing");
  assert.equal(legacy.suggestions?.exclusions.value.length, 0);
  assert.equal(validateProfileAiTargetingQuality(legacy.suggestions).valid, false);
});
