...
test("added account before connect", () => {
  ...
  assert.equal(ui.connectLabel, "Connecter le compte");
...
test("ready selected account exposes one verify-and-connect action", () => {
  const ui = resolveClientAccountState({
    loginStatus: "pending",
...
test("mixed connected and disconnected accounts keep independent actions", () => {
  const disconnected = resolveClientAccountState({
    loginStatus: "pending",
    onboardingStatus: "configured",
    assignmentStatus: "assigned",
    clientReadinessStatus: "ready_to_connect",
  }, "fr");
  const connected = resolveClientAccountState({
    loginStatus: "connected",
    onboardingStatus: "ready",
    assignmentStatus: "assigned",
    connected: true,
  }, "fr");
  assert.equal(disconnected.connectLabel, "Vérifier et connecter");
  assert.equal(disconnected.connectDisabled, false);
  assert.equal(connected.connectLabel, "Connecté");
  assert.equal(connected.connectDisabled, true);
});
