...
test("avatar proxy server tries fresh SearchAPI avatar before giving up", () => {
  const proxySource = source("./target-avatar-proxy-server.ts");
  assert.match(proxySource, /lookupInstagramPublicProfile/);
  assert.match(proxySource, /Referer: "https:\/\/www.instagram.com\/"/);
});
