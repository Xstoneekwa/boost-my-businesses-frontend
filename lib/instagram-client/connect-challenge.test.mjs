...
test("wrong-password action wins over generic verification state", () => {
  const snapshot = projectClientConnectProgress({
    accountId: "acc-wrong-password",
    overallStatus: "action_required",
    ...
    actionRequired: {
      id: "password-action-1",
      action_type: "update_instagram_password",
      status: "pending",
      title: "Mot de passe Instagram à mettre à jour",
      message: "Instagram a refusé les identifiants de ce compte. Mettez à jour le mot de passe pour reprendre la connexion en toute sécurité.",
      accepts_verification_code: false,
    },
    lang: "fr",
  });

  assert.equal(snapshot.connect_status, "wrong_password");
  assert.equal(snapshot.action_required?.action_type, "update_instagram_password");
  assert.equal(snapshot.action_required?.can_submit_code, false);
  assert.equal(snapshot.action_required?.title, "Mot de passe Instagram à mettre à jour");
  assert.match(snapshot.message, /Mettez à jour le mot de passe/);
  assert.equal(isTerminalClientConnectProgress(snapshot), true);
});

test("wrong-password projection remains stable across refresh", () => {
  const input = {
    accountId: "acc-wrong-password",
    overallStatus: "action_required",
    actionRequired: {
  ...
  const first = projectClientConnectProgress(input);
  const refreshed = projectClientConnectProgress(input);

  assert.equal(first.connect_status, "wrong_password");
  assert.equal(refreshed.connect_status, first.connect_status);
  assert.deepEqual(refreshed.action_required, first.action_required);
});
...
test("human review remains action-required without becoming code-submittable", () => {
  const snapshot = projectClientConnectProgress({
    accountId: "acc-human-review",
    overallStatus: "action_required",
    actionRequired: {
      id: "human-review-1",
      action_type: "review_login_challenge",
      status: "pending",
      title: "Confirmation manuelle requise",
      message: "Confirmez cette connexion dans Instagram.",
      accepts_verification_code: false,
    },
    lang: "fr",
  });
  ...
  assert.equal(snapshot.action_required?.action_type, "review_login_challenge");
  assert.equal(snapshot.action_required?.can_submit_code, false);
});
...
test("missing projection is transient and never maps to active running status", () => {
  const snapshot = projectClientConnectProgress({
    accountId: "acc-1",
...
test("wrong-password modal is action-specific and code UI is fail-closed", () => {
  assert.match(processModal, /data-modal-type=\{isWrongPassword \? "wrong_password"/);
  assert.match(processModal, /"Mettre à jour le mot de passe"/);
  assert.match(processModal, /!isWrongPassword && canEnterCode/);
  assert.match(verificationModal, /!canSubmitCode && !resumeInProgress/);
});

test("verification modal opening is gated by code semantics", () => {
  assert.match(clientSection, /shouldOpenVerificationModal/);
  assert.match(clientSection, /canSubmitVerificationCode\(snapshot\.action_required\)/);
  assert.doesNotMatch(clientSection, /connect_status === "verification_required"\s*\|\|\s*processModal\.connectProgress/);
});
