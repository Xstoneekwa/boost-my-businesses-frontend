...
test("terminal historical password action does not reactivate wrong-password state", () => {
  const snapshot = projectClientConnectProgress({
    accountId: "acc-historical-password",
    overallStatus: "ready_to_connect",
    actionRequired: {
      id: "password-action-old",
      action_type: "update_instagram_password",
      status: "dismissed",
      accepts_verification_code: false,
    },
  ...
  assert.notEqual(snapshot.connect_status, "wrong_password");
...
test("wrong-password modal exposes only the password action", () => {
