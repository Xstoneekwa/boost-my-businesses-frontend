...
function publicAnalysis(overrides = {}) {
  return {
    lookupStatus: "found",
...
test("public analysis UI distinguishes complete, partial, retryable and not-found states", () => {
  assert.equal(publicAnalysisUiState(publicAnalysis()), "complete");
  assert.equal(publicAnalysisUiState(publicAnalysis({ avatarAvailable: false })), "partial");
  assert.equal(publicAnalysisUiState(publicAnalysis({ lookupStatus: "rate_limited" })), "failed_retryable");
  assert.equal(publicAnalysisUiState(publicAnalysis({ lookupStatus: "provider_error" })), "failed_retryable");
  assert.equal(publicAnalysisUiState(publicAnalysis({ lookupStatus: "not_found" })), "not_found");
  assert.match(publicAnalysisStateCopy("fr", "failed_retryable"), /session et les identifiants sont conservés/);
  assert.match(publicAnalysisStateCopy("fr", "not_found"), /introuvable/);
});

test("an entirely unknown analysis cannot be confirmed and null followers never become zero", () => {
  const empty = publicAnalysis({
  ...
  assert.equal(hasConfirmablePublicAnalysis(empty), false);
  assert.equal(empty.followersCount, null);
  assert.equal(hasConfirmablePublicAnalysis({ ...empty, niche: "Niche confirmée" }), true);
});
...
test("wizard renders long lists as wrapped multiline controls without automatic AI calls", () => {
  ...
  assert.equal((wizard.match(/className="cio-targeting-multiline"/g) ?? []).length, 7);
  assert.match(wizard, /Whitelist — comptes à protéger/);
  assert.match(wizard, /Blacklist — comptes à exclure/);
