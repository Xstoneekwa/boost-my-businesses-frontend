...
create table public.client_instagram_accounts (
  ...
  label text, onboarding_status text, provisioning_status text, login_status text, updated_at timestamptz default now()
);
...
function beginSql(attemptId) {
  return `select public.begin_client_instagram_onboarding(
    '${IDS.client}', '${IDS.actor}', '${IDS.entitlement}', '${IDS.idempotency}', '${attemptId}',
...
test("PostgreSQL RPC is atomic, concurrent, resumable, and enforces the DB target count", { timeout: 90_000 }, async (t) => {
  ...
  const accountId = await query(port, `select account_id from public.client_instagram_onboarding_sessions
    where idempotency_key = '${IDS.idempotency}'`);
  await query(port, `select public.advance_client_instagram_onboarding('${sessionId}', '${IDS.client}', '${IDS.actor}', 'save_analysis', '{"username":"atomic_test_account"}'::jsonb)`);
  for (const listKind of ["unfollow_whitelist", "interaction_blacklist"]) {
    const mutation = JSON.parse(await query(port, `select public.mutate_account_protection_list(
      '${accountId}', '${listKind}', 'replace', array[]::text[], array[]::text[], array[]::text[],
      'client_dashboard', '${IDS.actor}', gen_random_uuid()::text, gen_random_uuid()::text, 0,
      encode(digest('${listKind}', 'sha256'), 'hex')
    )::text`));
    assert.equal(mutation.ok, true);
  }
  await query(port, `select public.advance_client_instagram_onboarding('${sessionId}', '${IDS.client}', '${IDS.actor}', 'save_protection_lists', '{}'::jsonb)`);
  ...
  assert.equal(await count(port, "public.client_instagram_onboarding_sessions", "status = 'active'"), 1);
