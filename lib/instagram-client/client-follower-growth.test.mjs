...
test("no snapshots → empty state without stale ig_accounts fallback", () => {
  ...
  assert.equal(series.freshnessStatus, "pending");
...
test("single snapshot → current followers visible, delta unknown, no curve", () => {
  ...
  assert.equal(series.freshnessStatus, "stale");
  assert.equal(view.mainValue, "28");
  assert.equal(view.deltaDisplay, "—");
  assert.equal(view.showChart, false);
  assert.match(view.subtitle, /Données anciennes — dernier relevé/);
});
...
test("fresh single snapshot is explicitly insufficient and source failures are explicit", () => {
  const series = projectClientFollowerGrowthSeries({
    accountId: "acct-a",
    snapshots: [snap("2026-06-20T08:00:00.000Z", 28, "baseline")],
    clientLinkedAt: CLIENT_LINK,
    businessTimezone: TZ,
    period: "all",
    now: NOW,
  });
  assert.equal(series.freshnessStatus, "insufficient");

  const unavailable = buildFollowerChartView({ ...series, freshnessStatus: "unavailable" }, "fr");
  const error = buildFollowerChartView({ ...series, freshnessStatus: "error" }, "fr");
  assert.equal(unavailable.subtitle, "Données abonnés indisponibles");
  assert.equal(error.subtitle, "Erreur de lecture des abonnés");
});
...
test("daily without intraday snapshots → honest empty chart", () => {
  ...
  const view = buildFollowerChartView(series, "fr");
  assert.equal(view.showChart, false);
  assert.match(view.subtitle, /Historique intrajournalier indisponible/);
});
