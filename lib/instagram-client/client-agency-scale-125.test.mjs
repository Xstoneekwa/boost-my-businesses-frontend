...
test("agency-wide scope hides individual subscription cards and mono-account follower charts", () => {
  ...
  assert.match(dashboardSource, /hasOverviewInsights = Boolean\(accountInsights\)/);
  assert.match(dashboardSource, /accountInsights\?\.accountId === overviewScope/);
  assert.match(dashboardSource, /selectedOverviewAccountId = agencyModeActive/);
  assert.match(dashboardSource, /useAccountScopedSubscription = Boolean\(selectedOverviewAccountId\)/);
