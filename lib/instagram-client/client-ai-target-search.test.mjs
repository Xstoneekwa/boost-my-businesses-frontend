...
test("google query builder produces manual-style Johannesburg chinese restaurant queries", async () => {
  const { buildTargetAiGoogleQueries } = await import("./target-ai-google-query-builder.ts");
  ...
  assert.match(sample, /"Johannesburg"/i);
  assert.match(sample, /site:instagram\.com/);
  assert.match(sample, /-inurl:\/p\//);
  assert.match(sample, /-inurl:\/explore/);
  assert.ok(
    queries.some((query) => query.includes("restaurant chinois") || query.includes("chinese restaurant")),
  );
});

test("manual benchmark queries match effective formats", async () => {
  ...
  assert.ok(jhb.some((query) => query.includes('"restaurant" "chinois"')));
  const psycho = buildTargetAiManualBenchmarkQueries({
    niche: "psychologue",
    locationLabel: "Johannesburg",
  });
  assert.ok(psycho.some((query) => query.includes("clinical psychologist")));
  const belgium = buildTargetAiManualBenchmarkQueries({
    niche: "agence social media",
    locationLabel: "Belgique",
  });
  assert.ok(belgium.some((query) => query.includes("Bruxelles") || query.includes("bruxelles")));
  assert.ok(belgium.some((query) => query.includes("Brussels") || query.includes("brussels")));
});

test("google query builder injects Belgium major cities", async () => {
  const { buildTargetAiGoogleQueries } = await import("./target-ai-google-query-builder.ts");
  const queries = buildTargetAiGoogleQueries({
    niche: "agence social media",
    locationLabel: "Belgique",
  ...
  assert.ok(joined.includes("belgique") || joined.includes("belgium"));
  assert.ok(joined.includes("bruxelles") || joined.includes("brussels"));
});

test("client copy never mentions Google or SearchAPI", () => {
...
test("google query builder includes Bordeaux asiatique variants", async () => {
  const { buildTargetAiGoogleQueries } = await import("./target-ai-google-query-builder.ts");
  const queries = buildTargetAiGoogleQueries({
    niche: "restaurant asiatique",
    locationLabel: "Bordeaux, Gironde, France",
  ...
  assert.ok(queries.some((query) => query.includes('"Bordeaux"')));
  assert.ok(queries.some((query) => query.includes("restaurant asiatique") || query.includes("restaurant japonais")));
});
...
test("SERP scorer ranks niche and location matches higher", async () => {
  const { rankSerpProfileCandidates } = await import("./target-ai-serp-score.ts");
  const ranked = rankSerpProfileCandidates([
    {
      username: "generic_food",
      profileUrl: "https://www.instagram.com/generic_food/",
      title: "Food inspiration",
      snippet: "National recipes",
      displayedLink: null,
      sourceQuery: '"Johannesburg" food site:instagram.com',
      position: 8,
    },
    {
      username: "jhb_chinese_kitchen",
      profileUrl: "https://www.instagram.com/jhb_chinese_kitchen/",
      title: "Johannesburg Chinese Kitchen",
      snippet: "Chinese restaurant in Johannesburg",
      displayedLink: null,
      sourceQuery: '"Johannesburg" "restaurant chinois" site:instagram.com',
      position: 2,
    },
  ], "restaurant chinois", "Johannesburg");
  assert.equal(ranked[0].username, "jhb_chinese_kitchen");
});

test("V2 search service is default entrypoint", () => {
  const serviceSource = source("./target-ai-search-service.ts");
  const v2Source = source("./target-ai-search-v2-service.ts");
  assert.match(serviceSource, /searchTargetAccountsWithAiV2/);
  assert.match(v2Source, /google_first_v2/);
...
test("AI wizard supports verify selection before validate", () => {
  const wizardSource = source("../../app/instagram-client/ClientAiTargetSearchWizard.tsx");
  assert.match(wizardSource, /ai-search\/verify/);
  assert.match(wizardSource, /validateSelection/);
  assert.match(wizardSource, /suggestedProfile/);
  assert.doesNotMatch(wizardSource, /verifySelection/);
});
