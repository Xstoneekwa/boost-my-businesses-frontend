...
test("google query builder injects Belgium major cities in loose queries", async () => {
  ...
  assert.ok(joined.includes("antwerp") || joined.includes("antwerpen"));
  assert.ok(joined.includes("gent") || joined.includes("ghent") || joined.includes("liege") || joined.includes("liège"));
});
...
test("V2 search service uses SearchAPI loose discovery without initial profile lookup", () => {
  const serviceSource = source("./target-ai-search-service.ts");
  ...
  assert.match(v2Source, /TARGET_AI_V2_AUTO_VERIFY_COUNT", 0/);
