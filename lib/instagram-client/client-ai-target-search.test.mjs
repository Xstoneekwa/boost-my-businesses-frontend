...
test("target AI search service separates GPT seeds from SearchAPI verification", () => {
  const serviceSource = source("./target-ai-search-service.ts");
  assert.match(serviceSource, /buildTargetAiSystemPrompt/);
  assert.match(serviceSource, /verifySingleTargetUsername/);
  assert.match(serviceSource, /secondPassUsed/);
  assert.match(serviceSource, /verificationStatus === "found"/);
  assert.match(serviceSource, /targetingAiPromptVersion/);
});

test("targeting AI admin config route exposes prompt without secrets", () => {
  const configRoute = source("../../app/api/instagram-dashboard/targeting-ai/config/route.ts");
  const healthRoute = source("../../app/api/instagram-dashboard/targeting-ai/health/route.ts");
  assert.match(configRoute, /verifyCompassRelayKey/);
  assert.match(configRoute, /buildTargetingAiPublicConfig/);
  assert.match(configRoute, /user_template_preview/);
  assert.doesNotMatch(configRoute, /process\.env\.OPENAI_API_KEY/);
  assert.match(healthRoute, /searchapi_key_configured/);
  assert.doesNotMatch(healthRoute, /INSTAGRAM_PUBLIC_PROFILE_LOOKUP_API_KEY/);
});
