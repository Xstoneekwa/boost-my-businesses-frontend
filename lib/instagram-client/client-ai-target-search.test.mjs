...
test("target AI profile lookup concurrency stays throttled at two workers max", async () => {
  ...
  assert.equal(readTargetAiProfileLookupConcurrency(4, 4), 1);
});
...
test("target AI runtime stops when max latency is exceeded", async () => {
  ...
  const runtime = new TargetAiSearchRuntime({
    maxLatencyMs: 50,
    primaryQueryLimit: 10,
    broadenedQueryLimit: 8,
    complementaryQueryLimit: 6,
    rateLimitCooldownMs: 1000,
    minCandidateScore: 0,
    targetEligibleCount: 12,
    thirdPassEnabled: true,
    maxDiscoveredUsernames: 50,
    maxProfileChecks: 60,
  }, Date.now() - 60);
  assert.equal(runtime.isTimeExceeded(), true);
  runtime.markStopped("time_budget");
  assert.equal(runtime.stoppedReason, "time_budget");
});

test("target AI runtime treats repeated rate limits as severe", async () => {
  const { TargetAiSearchRuntime } = await import("./target-ai-search-runtime.ts");
  const runtime = new TargetAiSearchRuntime({
    maxLatencyMs: 120000,
    primaryQueryLimit: 10,
  ...
  assert.equal(runtime.isRateLimitSevere(), true);
