...
test("canonical onboarding persists account-scoped protection before targeting", () => {
  ...
  assert.match(wizard, /Protection & ciblage/);
  assert.match(wizard, /whitelist: protectedAccounts, blacklist: blockedAccounts/);
  assert.match(wizard, /filtersResponse[\s\S]*save_targeting/);
  assert.match(filtersRoute, /authorizeClientInstagramAccount/);
  assert.match(filtersRoute, /\.eq\("account_id", accountId\)/);
