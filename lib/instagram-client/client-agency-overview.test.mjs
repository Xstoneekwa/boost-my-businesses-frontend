...
test("agency mode activates only with two or more linked accounts", () => {
  ...
  assert.equal(isAgencyModeActive(6), true);
});
...
test("dashboard keeps single-account overview unchanged and adds agency scope flow", () => {
  ...
  assert.match(projectionSource, /projectCommercialLifecyclePresentation\(input\.account\.accountStatus, "fr"\)/);
  assert.match(projectionSource, /campaignLabelFr:\s*lifecycleFr\?\.label/);
  assert.match(projectionSource, /campaignLabelEn:\s*lifecycleEn\?\.label/);
});
