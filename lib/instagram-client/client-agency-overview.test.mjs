...
const actionScopeSource = readFileSync(new URL("./client-connection-action-scope.ts", import.meta.url), "utf8");
...
test("dashboard keeps single-account overview unchanged and adds agency scope flow", () => {
  ...
  assert.match(dashboardSource, /resolveClientConnectionActionPanel/);
  assert.match(dashboardSource, /accountScopeId=\{connectionActionPanel\.accountScopeId\}/);
  assert.match(actionScopeSource, /account\.accountId === input\.overviewScope/);
});
