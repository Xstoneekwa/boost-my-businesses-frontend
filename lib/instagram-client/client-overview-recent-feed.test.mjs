...
test("overview uses product-standard Follow and Unfollow labels in FR and EN", () => {
  const feed = buildClientOverviewRecentFeed([
    event({ id: "follow", event_type: "follow_verified_persisted_v1", interaction_type: "follow" }),
    event({
      id: "unfollow",
      event_type: "unfollow_verified",
      interaction_type: "unfollow",
      source_target_username: null,
      username: "former_follow",
      event_at: "2026-06-08T20:55:00.000Z",
    }),
  ], { accountId: ACCOUNT_A, businessTimezone: TZ });

  const follow = feed.find((item) => item.actionKind === "follow");
  const unfollow = feed.find((item) => item.actionKind === "unfollow");
  assert.equal(follow?.categoryLabelFr, "Follows");
  assert.equal(follow?.categoryLabelEn, "Follows");
  assert.match(follow?.summaryFr ?? "", /1 Follow envoyé/);
  assert.equal(unfollow?.categoryLabelFr, "Unfollows");
  assert.equal(unfollow?.categoryLabelEn, "Unfollows");
  assert.equal(unfollow?.summaryFr, "1 Unfollow");
  assert.equal(unfollow?.summaryEn, "1 Unfollow");
  assert.doesNotMatch(unfollow?.summaryFr ?? "", /campagne|cible/i);
});
