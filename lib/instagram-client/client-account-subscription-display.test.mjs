...
test("account scoped subscription card titles username and uses account package only", () => {
  ...
  assert.match(growthCard.title, /Plan du compte Instagram — @nab_autom_ig/);
...
test("dashboard wires account scoped subscription for mono and agency account views", () => {
  ...
  assert.match(dashboardSource, /initialAccountSubscription/);
  assert.match(pageSource, /loadAccountCommercialSubscriptionDisplay/);
  assert.match(pageSource, /initialAccountSubscription=/);
  assert.match(dashboardSource, /Plan du tenant/);
});
...
test("loadAccountCommercialSubscriptionDisplay is exported for tenant-scoped API", async () => {
  const loadedModule = await import("./load-account-commercial-subscription.ts");
  assert.equal(typeof loadedModule.loadAccountCommercialSubscriptionDisplay, "function");
});
