...
const accountSubscriptionSource = readFileSync(new URL("./load-account-commercial-subscription.ts", import.meta.url), "utf8");
...
test("resolveSubscriptionPeriodEnd never synthesizes a date from activation and duration", () => {
...
test("account billing display never restores a future period date after a canonical payment issue", () => {
  assert.match(accountSubscriptionSource, /const billingDateIso = billingDate\?\.dateIso \|\| ""/);
  assert.doesNotMatch(accountSubscriptionSource, /billingDate\?\.dateIso \|\| periodEndAt/);
});
...
test("projectClientSubscriptionDisplay builds Pro checkout simulation view without runtime labels", () => {
  ...
  assert.equal(projection.subscriptionPeriodEnd, null);
...
test("subscription overview card uses the canonical next-payment label", () => {
  ...
  assert.equal(card.billingDateLabel, "Prochain prélèvement");
