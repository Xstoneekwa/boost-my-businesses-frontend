...
test("client dashboard UI exposes one add CTA per context", () => {
  ...
  assert.match(dashboardSource, /hasLinkedInstagramAccount && activeView === "overview"/);
  assert.match(dashboardSource, /!hasLinkedInstagramAccount && activeView === "overview"/);
});
...
test("client dashboard keeps non-overview views visible without linked instagram account", () => {
  ...
  assert.match(dashboardSource, /t\.activity\.emptyBody/);
  assert.match(dashboardSource, /t\.targeting\.emptyBody/);
