...
test("client dashboard UI exposes one add CTA per context", () => {
  ...
  assert.match(dashboardSource, /displayMode=\{agencyModeActive \? "add_only" : "accounts"\}/);
...
test("agency add CTA keeps entitlement routing, subscription enforcement, auth, and responsive visibility", () => {
  const sectionSource = source("../../app/instagram-client/ClientAccountsSection.tsx");
  const dashboardSource = source("../../app/instagram-client/ClientDashboard.tsx");
  const createSource = source("./create-account.ts");
  const sessionSource = source("./_utils.ts");

  assert.match(sectionSource, /if \(entitlementReady\) \{\s*setFormOpen\(true\)/);
  assert.match(sectionSource, /router\.push\("\/instagram-client\/choose-plan"\)/);
  assert.match(createSource, /if \(!subscriptionActive\)/);
  assert.match(createSource, /code: "subscription_inactive"/);
  assert.match(createSource, /code: "entitlement_required"/);
