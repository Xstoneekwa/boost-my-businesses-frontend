...
test("client onboarding route requires tenant session and rejects technical fields", () => {
  ...
  assert.match(onboardingRoute, /beginInstagramAccountOnboarding/);
...
test("client workspace and insights routes are tenant-safe", () => {
  ...
  assert.match(workspaceData, /accountStatus:\s*projected\.accountStatus/);
  assert.match(workspaceData, /projectCommercialLifecyclePresentation\(projected\.accountStatus, preferredLanguage\)/);
});

test("client account surfaces prioritize canonical paused and cancelled lifecycle over login and readiness", () => {
  const sectionSource = source("../../app/instagram-client/ClientAccountsSection.tsx");
  const dashboardSource = source("../../app/instagram-client/ClientDashboard.tsx");
  assert.match(sectionSource, /projectCommercialLifecyclePresentation\(account\.accountStatus, lang\)/);
  assert.match(sectionSource, /lifecycle\?\.label \?\? ui\.badgeLabel/);
  assert.match(dashboardSource, /projectCommercialLifecyclePresentation\(account\.accountStatus, lang\)/);
  assert.match(dashboardSource, /lifecycleStatus\?\.label \?\? \(account\.connected/);
});
...
test("create account keeps internal dry-run support but client onboarding cannot request it", () => {
  ...
  const onboardingRoute = source("../../app/api/instagram-client/onboarding/route.ts");
  const adminRoute = source("../../app/api/instagram-dashboard/accounts/create/route.ts");
  assert.match(createSource, /peekReservedEntitlementForClient/);
  assert.match(createSource, /entitlement_required/);
  assert.match(createSource, /legacy_direct_create_disabled/);
  ...
  assert.match(adminRoute, /previewInstagramAccountOnboarding/);
  assert.match(adminRoute, /body\.dry_run === true/);
});

test("canonical onboarding is governed by per-account entitlement, not a stale global account limit", () => {
  const createSource = source("./create-account.ts");
  assert.doesNotMatch(createSource, /clientMaxAccountsLimit|max_accounts_reached|INSTAGRAM_CLIENT_MAX_ACCOUNTS/);
  assert.match(createSource, /peekReservedEntitlementForClient/);
  assert.match(createSource, /entitlement_required/);
});
...
test("client add account persists login email through shared helper", () => {
  ...
  const transactionSource = source("../../supabase/migrations/20260721120000_client_instagram_onboarding_sessions.sql");

  assert.match(onboardingRoute, /parseLoginEmailInput/);
  assert.match(onboardingRoute, /email_invalid/);
  assert.match(onboardingRoute, /email:\s*email\.email \?\? ""/);
  assert.match(transactionSource, /insert into public\.ig_account_settings/);
  assert.match(transactionSource, /lower\(trim\(coalesce\(p_login_email, ''\)\)\)/);
  assert.match(persistSource, /normalizeSafeEmail/);
  assert.doesNotMatch(transactionSource, /i_m_your_traker|growth_with_bmb/);
});
...
test("client add account credentials ingestion uses client actor attribution", () => {
  ...
  assert.match(onboardingRoute, /actorType:\s*"client"/);
  assert.match(onboardingRoute, /source:\s*"client_dashboard"/);
  assert.match(canonicalMigration, /p_actor_type/);
  assert.match(canonicalMigration, /p_source_surface/);
  assert.doesNotMatch(onboardingRoute, /body\.actor|body\.source_surface/);
  assert.doesNotMatch(onboardingRoute, /console\.(log|info|warn|error)\([\s\S]*password/);
});
...
test("client add account rolls back ig_accounts after credentials ingestion failure", () => {
  ...
  const transactionSource = source("../../supabase/migrations/20260721120000_client_instagram_onboarding_sessions.sql");
  assert.match(onboarding, /begin_instagram_account_onboarding_v1/);
  assert.match(transactionSource, /insert into public\.ig_accounts/);
  assert.match(transactionSource, /create_instagram_credentials_vault_secret/);
  assert.match(transactionSource, /insert into public\.client_instagram_accounts/);
  assert.match(transactionSource, /update public\.client_account_entitlements/);
  assert.match(transactionSource, /exception\s+when others then/);
  assert.doesNotMatch(onboarding, /\.from\("ig_accounts"\)\.delete/);
});
...
test("client add account without email stays optional", () => {
  ...
  const transactionSource = source("../../supabase/migrations/20260721120000_client_instagram_onboarding_sessions.sql");
  assert.match(onboardingRoute, /email:\s*email\.email \?\? ""/);
  assert.match(transactionSource, /coalesce\(p_login_email, ''\)/);
  assert.doesNotMatch(onboardingRoute, /email_required/);
});
