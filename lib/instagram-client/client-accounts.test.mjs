...
test("client dashboard UI exposes one add CTA per context", () => {
  ...
  assert.match(sectionSource, /confirmVerifiedConnection/);
  assert.match(sectionSource, /confirmed: true/);
  assert.match(sectionSource, /connectSubmissionRef/);
  assert.match(sectionSource, /runConnectProcess\(account, "check_readiness"\)/);
  ...
  assert.match(modalSource, /Connecter maintenant|Connect now/);
