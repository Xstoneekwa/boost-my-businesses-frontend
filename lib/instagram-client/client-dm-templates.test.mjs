...
test("multi-account client projection keeps Growth locked and Pro editable", () => {
  const growth = projectClientDmTemplates({
    accountId: "acct-growth",
    username: "growth_user",
    packageCode: "growth",
    domain: domainProjection({ welcome_service_active: false }),
  });
  const pro = projectClientDmTemplates({
    accountId: "acct-pro",
    username: "pro_user",
    packageCode: "pro",
    domain: domainProjection({ welcome_service_active: true, welcome_entitlement_status: "Included" }),
  });
  assert.equal(growth.canConfigureWelcome, false);
  assert.equal(pro.canConfigureWelcome, true);
});
...
test("client dm routes enforce session, ownership and locked writes", () => {
  ...
  const dmCapacity = source("./account-dm-capacity.ts");
  const domainService = source("../instagram-dashboard/dm-domain-service.ts");
  ...
  assert.match(getRoute, /sanitizeClientApiError/);
  assert.doesNotMatch(getRoute, /client_instagram_accounts.*username/);
  assert.match(loaderSource, /loadCanonicalIgAccountUsername/);
  assert.match(dmCapacity, /resolveAccountWelcomeServiceActive/);
...
test("create account auto-assign defers when live capacity is unavailable", () => {
