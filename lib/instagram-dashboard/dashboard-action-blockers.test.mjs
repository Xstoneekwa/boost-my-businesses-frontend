...
test("a linked action blocks only while its incident is active", () => {
  const linked = {
  ...
  assert.equal(isCurrentBlockingDashboardAction(linked, {
    now,
    activeIncidentIds: new Set(["incident-active"]),
  }), true);
  assert.equal(isCurrentBlockingDashboardAction(linked, {
    now,
    activeIncidentIds: new Set(),
  }), false);
  assert.equal(isCurrentBlockingDashboardAction({ ...linked, incident_id: null }, {
    now,
    activeIncidentIds: new Set(),
  }), true);
});
