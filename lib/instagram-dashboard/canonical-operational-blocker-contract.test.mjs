...
const lineageRepairMigration = readFileSync(
  new URL("../../supabase/migrations/20260901193835_incident_dashboard_action_lineage_repair_v1.sql", import.meta.url),
  "utf8",
);
...
test("incident-derived actions preserve exact lineage without weakening standalone blockers", () => {
  assert.match(lineageRepairMigration, /p_incident_id uuid default null/);
  assert.match(lineageRepairMigration, /p_incident_id := p_incident_id/);
  assert.match(lineageRepairMigration, /action\.incident_id is null/);
  assert.match(lineageRepairMigration, /incident\.account_id = action\.account_id/);
  assert.match(lineageRepairMigration, /incident\.run_id::text = action\.metadata ->> 'run_id'/);
  assert.match(lineageRepairMigration, /incident_matches = 1 and action_matches = 1/);
  assert.match(lineageRepairMigration, /interval '5 seconds'/);
  assert.doesNotMatch(lineageRepairMigration, /update public\.account_dashboard_actions[\s\S]*?where action\.incident_id is not null/);
  assert.doesNotMatch(lineageRepairMigration, /delete from public\.account_(?:incidents|dashboard_actions)/);
});
