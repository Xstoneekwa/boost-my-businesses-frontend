...
test("detail contract is versioned, tolerant and redacted", () => {
  ...
  assert.match(detailSource, /resolutionNote: row\.note/);
});
...
test("resolution notification deduplication and retries are channel-specific and bounded", () => {
  ...
  assert.match(actionRoute, /deliveries = \[\{ status: "queued", router: "notification_router_v2" \}\]/);
  assert.doesNotMatch(actionRoute, /delivery_dispatch_failed_after_database_commit/);
});
...
test("manual resolution V3 keeps runtime proof for ordinary incidents and returns explicit recovery fields", () => {
  assert.match(actionRoute, /transition_account_incident_human_review_v3/);
  assert.match(actionRoute, /action === "resolve" && !incidentOnlyActionLimit/);
...
test("incident-only action limits do not fabricate runtime-fix proof or resume authorization", () => {
  assert.match(actionRoute, /isIncidentOnlyActionRateLimit/);
  assert.match(actionRoute, /p_expected_worker_sha: action === "resolve" && !incidentOnlyActionLimit \? expectedWorkerSha : null/);
  assert.match(actionRoute, /p_cause_fixed_version: action === "resolve" && !incidentOnlyActionLimit \? causeFixedVersion : null/);
  assert.match(actionRoute, /early_resolution_warning: incidentOnlyActionLimit/);
});
