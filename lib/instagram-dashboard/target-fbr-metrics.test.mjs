...
test("backfill migration is positive-only and skips zero without coverage", async () => {
  const { readFileSync } = await import("node:fs");
  const sql = readFileSync(
    new URL("../../supabase/migrations/20260615190000_sync_target_followbacks_count.sql", import.meta.url),
    "utf8",
  );
  assert.match(sql, /p_certify_zero_coverage/);
  assert.match(sql, /no_positive_attribution_without_scan_coverage/);
  assert.match(sql, /Positive-only/);
  assert.match(sql, /certified_targets/);
  assert.doesNotMatch(sql, /followbacks_metrics_reliable_at = v_now\s*\n\s*where id = p_target_id;\s*\n\s*return jsonb_build_object\([\s\S]*?'followbacks_count', v_count,\s*\n\s*'followbacks_metrics_reliable_at', v_now/s);
});

test("zero followback without coverage stays non reliable in display projection", () => {
