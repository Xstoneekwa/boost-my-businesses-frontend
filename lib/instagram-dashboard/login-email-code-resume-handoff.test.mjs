...
test("resume metadata preserves every canonical verification channel without email fallback", () => {
  for (const channel of ["email", "sms", "whatsapp", "authenticator_app"]) {
    ...
    assert.equal(metadata.verification_channel, channel);
    assert.equal(metadata.challenge_type, `${channel}_code_challenge`);
  }

  const unknown = buildLoginEmailCodeResumeMetadata({
    actionId,
    accountId,
    submissionId,
    verificationChannel: "unknown",
  });
  assert.equal(unknown.verification_channel, undefined);
  assert.equal(unknown.challenge_type, "verification_code_challenge");
  assert.notEqual(unknown.challenge_type, "email_code_challenge");
  assert.equal(normalizeVerificationChannel("SMS"), "sms");
  assert.equal(normalizeVerificationChannel("whatsapp_code_challenge"), "whatsapp");
  assert.equal(normalizeVerificationChannel("unknown"), null);
});
