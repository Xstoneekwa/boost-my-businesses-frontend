...
test("the cap RPC updates legacy settings and provenance atomically while package-equal values clear the override", () => {
...
test("read-only Follow provenance projections never leak into ig_account_settings writes", () => {
  const projectionKeys = settingsRoute.match(
    /const runtimeProjectionKeys = \[([\s\S]*?)\] as const;/,
  )?.[1] ?? "";

  assert.match(projectionKeys, /"follow_account_override_status"/);
  assert.match(projectionKeys, /"follow_account_override_source"/);
  assert.match(projectionKeys, /"manual_follow_day_cap"/);
  assert.match(projectionKeys, /"manual_follow_session_cap"/);
  assert.match(settingsRoute, /for \(const key of runtimeProjectionKeys\) \{\s*delete payload\[key\];\s*\}/);
  assert.doesNotMatch(migration, /alter table public\.ig_account_settings[\s\S]*add column[^;]*follow_account_override_(?:status|source)/i);
});
