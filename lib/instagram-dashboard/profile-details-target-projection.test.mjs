...
import { resolveTargetAddedAt } from "./target-added-date.ts";
...
test("profile details Added uses created_at and ignores updated_at", () => {
  const createdAt = "2026-07-04T09:00:00.000Z";
  assert.equal(resolveTargetAddedAt({
    created_at: createdAt,
    updated_at: "2026-07-22T18:00:00.000Z",
  }), createdAt);
});

test("profile details Added stays stable across refresh updates", () => {
  const row = { created_at: "2026-07-04T09:00:00.000Z" };
  assert.equal(resolveTargetAddedAt({ ...row, updated_at: "2026-07-19T10:00:00.000Z" }), row.created_at);
  assert.equal(resolveTargetAddedAt({ ...row, updated_at: "2026-07-22T18:00:00.000Z" }), row.created_at);
});

test("profile details Added remains unknown when created_at is absent", () => {
  assert.equal(resolveTargetAddedAt({ created_at: null, updated_at: "2026-07-22T18:00:00.000Z" }), null);
});
