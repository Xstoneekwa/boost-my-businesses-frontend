...
import { parseProtectionListText } from "./account-protection-list-input.ts";
...
const sharedRoutes = await readFile(new URL("./account-protection-list-routes.ts", import.meta.url), "utf8");
...
test("add multiple normalizes and preserves first appearance", () => {
  assert.deepEqual(normalizeProtectionPatch(["Zulu", "alpha"], []).add, ["zulu", "alpha"]);
});
...
test("replace accepts a complete normalized array in first-appearance order", () => {
  assert.deepEqual(normalizeProtectionUsernameEntries(["Two", "one"], "items").items, ["two", "one"]);
  assert.match(migration, /p_operation = 'replace'/);
});

test("duplicate normalized input is silently deduplicated", () => {
  const result = normalizeProtectionUsernameEntries(["Alpha", "@alpha"], "items");
  assert.deepEqual(result.items, ["alpha"]);
  assert.deepEqual(result.errors, []);
  assert.equal(result.duplicateCount, 1);
});

test("comma, newline, CRLF, mixed separators, spaces and optional @ share one parser", () => {
  const parsed = parseProtectionListText(" @Alpha, beta\r\ngamma , @alpha\n delta ");
  assert.deepEqual(parsed.items, ["alpha", "beta", "gamma", "delta"]);
...
test("weakly rewritten ETag remains a valid application version token", () => {
  const etag = accountProtectionListEtag(accountId, kind, 7);
  assert.deepEqual(readExpectedVersion(`W/${etag}`, accountId, kind), { ok: true, version: 7 });
  assert.match(sharedRoutes, /ACCOUNT_PROTECTION_LIST_VALIDATOR_HEADER/);
});
