...
test("verified Like progress is exposed by the existing asynchronous live projection", () => {
  const profile = projectProfilesLive({
    accountIds: ["account-1"],
    now: "2026-07-15T00:00:02.000Z",
    requests: [{ id: "request-1", account_id: "account-1", run_id: "run-1", status: "running", created_at: "2026-07-15T00:00:00.000Z" }],
    runs: [{ id: "run-1", account_id: "account-1", status: "running", started_at: "2026-07-15T00:00:00.000Z" }],
    actionLogs: [],
    interactionEvents: [{ account_id: "account-1", run_id: "run-1", event_type: "post_like_success", payload: { liked_count: 1 }, event_at: "2026-07-15T00:00:01.000Z" }],
    dashboardActions: [],
    followerSnapshots: [],
  })[0];

  assert.deepEqual(profile.liveSupportedKinds, ["follow", "unfollow", "like", "dm"]);
  assert.equal(profile.currentRunCounters.likes, 1);
});

test("verified Unfollow rows progress live without terminal double counting", () => {
  const input = {
    accountIds: ["account-1"],
    now: "2026-07-16T23:55:00.000Z",
  ...
  const rows = [1, 2, 3].map((index) => ({
    ...
    unfollowed_at: `2026-07-16T23:54:0${index}.000Z`,
  }));

  for (const count of [1, 2, 3]) {
    const profile = projectProfilesLive({ ...input, unfollowRows: rows.slice(0, count) })[0];
    assert.equal(profile.currentRunCounters.unfollows, count);
    assert.equal(profile.countersToday.unfollows, count);
  }

  const reconciled = projectProfilesLive({
    ...input,
    actionLogs: [{ account_id: "account-1", run_id: "run-1", target_username: "target-1", action_type: "unfollow_completed", status: "success" }],
    unfollowRows: rows.slice(0, 1),
  })[0];
  assert.equal(reconciled.currentRunCounters.unfollows, 1);
});

test("verified Unfollow rows remain canonical after the run terminates", () => {
  const unfollowRows = Array.from({ length: 6 }, (_, index) => ({
    id: `unfollow-${index}`,
    account_id: "account-1",
    run_id: "run-1",
    username: `target-${index}`,
  ...
  const profile = projectProfilesLive({
    accountIds: ["account-1"],
    now: "2026-07-16T23:59:00.000Z",
    requests: [],
    runs: [{ id: "run-1", account_id: "account-1", status: "completed" }],
    actionLogs: [], interactionEvents: [], unfollowRows, dashboardActions: [], followerSnapshots: [],
  })[0];
  assert.equal(profile.countersToday.unfollows, 6);
  assert.equal(profile.currentRunCounters.unfollows, 6);
});
...
test("rolling 72h follower delta reports +8 with complete coverage while freshness stays independent", () => {
  const profile = projectProfilesLive({
    accountIds: ["account-1"],
    now: "2026-07-21T17:00:00.000Z",
    requests: [], runs: [], actionLogs: [], interactionEvents: [], dashboardActions: [],
    followerSnapshots: [
      { account_id: "account-1", followers_count: 100, captured_at: "2026-07-17T00:30:00.000Z", source: "public_profile_lookup", observation_kind: "daily" },
      { account_id: "account-1", followers_count: 108, captured_at: "2026-07-20T00:30:00.000Z", source: "public_profile_lookup", observation_kind: "daily" },
    ],
  })[0];
  assert.deepEqual(profile.followerDelta3d, {
    ...
    from: "2026-07-17T00:30:00.000Z",
    to: "2026-07-20T00:30:00.000Z",
    source: "ig_account_follower_snapshots",
    windowCoverage: "complete",
    dataFreshness: "stale",
    latestSnapshotAt: "2026-07-20T00:30:00.000Z",
    baselineSnapshotAt: "2026-07-17T00:30:00.000Z",
    deltaFrom: "2026-07-17T00:30:00.000Z",
    deltaTo: "2026-07-20T00:30:00.000Z",
    staleAfterHours: 36,
  });
});
...
test("rolling 72h follower delta remains unknown when history is insufficient", () => {
  ...
  assert.equal(profile.followerDelta3d.windowCoverage, "insufficient_data");
  assert.equal(profile.followerDelta3d.dataFreshness, "fresh");
});

test("rolling 72h follower delta exposes partial coverage without inventing a value", () => {
  const profile = projectProfilesLive({
    accountIds: ["account-1"],
    now: "2026-07-21T12:00:00.000Z",
    requests: [], runs: [], actionLogs: [], interactionEvents: [], dashboardActions: [],
    followerSnapshots: [
      { account_id: "account-1", followers_count: 100, captured_at: "2026-07-19T00:30:00.000Z", source: "public_profile_lookup", observation_kind: "daily" },
      { account_id: "account-1", followers_count: 108, captured_at: "2026-07-21T00:30:00.000Z", source: "public_profile_lookup", observation_kind: "daily" },
    ],
  })[0];
  assert.equal(profile.followerDelta3d.value, null);
  assert.equal(profile.followerDelta3d.windowCoverage, "partial");
  assert.equal(profile.followerDelta3d.dataFreshness, "fresh");
});

test("light route is seven batched queries with no per-account loop queries", () => {
  const source = readFileSync(new URL("../../app/api/instagram-dashboard/profiles/live/route.ts", import.meta.url), "utf8");
  assert.match(source, /Promise\.all\(/);
  assert.match(source, /query_count:\s*7/);
  ...
  assert.equal((source.match(/supabase\.from\(/g) ?? []).length, 7);
});
