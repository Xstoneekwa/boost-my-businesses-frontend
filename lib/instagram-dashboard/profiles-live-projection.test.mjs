...
test("canonical run totals expose a monotone revision after DB acknowledgement", () => {
  const profile = projectProfilesLive({
    ...empty,
    accountIds: ["account-1"],
    now: "2026-07-23T00:00:05.000Z",
    ...
    runs: [{
      ...
      total_dm: 0,
      total_story: 0,
      live_counter_revision: 17,
      updated_at: "2026-07-23T00:00:04.000Z",
    }],
  })[0];
  assert.deepEqual(
    {
      runId: profile.currentRunCounters.runId,
      revision: profile.currentRunCounters.revision,
      follows: profile.currentRunCounters.follows,
      likes: profile.currentRunCounters.likes,
      source: profile.currentRunCounters.source,
      updatedAt: profile.currentRunCounters.updatedAt,
    },
    {
      runId: "run-1",
      revision: 17,
...
test("terminal projection retains the final canonical run revision", () => {
  const profile = projectProfilesLive({
    ...empty,
    accountIds: ["account-1"],
    ...
    runs: [{
      ...
      total_follow: 10,
      total_like: 9,
      live_counter_revision: 19,
      finished_at: "2026-07-23T00:00:05.000Z",
      updated_at: "2026-07-23T00:00:05.000Z",
    }],
  })[0];
  assert.equal(profile.runtimeIndicator.state, "idle");
  assert.equal(profile.currentRunCounters.runId, "run-1");
  assert.equal(profile.currentRunCounters.revision, 19);
  assert.equal(profile.currentRunCounters.likes, 9);
});
...
test("72h follower delta preserves real zero and leaves insufficient history null", () => {
  const complete = projectProfilesLive({
    ...
    socialProfileSnapshots: [
      { account_id: "account-1", followers_count: 53, following_count: 57, observed_at: "2026-07-19T23:00:00.000Z", lookup_status: "found", source_provider: "searchapi" },
      { account_id: "account-1", followers_count: 53, following_count: 58, observed_at: "2026-07-22T23:00:00.000Z", lookup_status: "found", source_provider: "searchapi" },
    ],
  })[0];
  ...
  const unknown = projectProfilesLive({
    ...
    accountIds: ["account-1"],
    now: "2026-07-23T00:00:00.000Z",
    socialProfileSnapshots: [{ account_id: "account-1", followers_count: 0, following_count: 0, observed_at: "2026-07-22T23:00:00.000Z", lookup_status: "found", source_provider: "searchapi" }],
  })[0];
