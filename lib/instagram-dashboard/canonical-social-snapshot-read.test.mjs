...
test('authoritative removal requires a coherent complete inventory, never partial or malformed Live',()=>{
 ...
 assert.deepEqual(canonicalProfilesMembership(payload,['a','deleted','removed']),{schema:'profiles_membership_v1',revision:now.toISOString(),removedAccountIds:['deleted']});
 ...
 assert.deepEqual(canonicalProfilesMembership({...payload,profiles:[],activeAccounts:[]},['a']).removedAccountIds,[]);
});
