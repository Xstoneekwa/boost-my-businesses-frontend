...
test("UUID validation rejects composite rows before RPC", () => {
  assert.equal(isUuid(operatorId), true);
  assert.equal(isUuid(`(${operatorId},resolved,{})`), false);
  assert.equal(isUuid({ id: operatorId }), false);
  assert.equal(isUuid([operatorId]), false);
});
