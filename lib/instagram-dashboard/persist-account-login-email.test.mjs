...
test("client onboarding route validates and forwards the optional login email", () => {
  assert.match(clientOnboardingRouteSource, /parseLoginEmailInput/);
  assert.match(clientOnboardingRouteSource, /email: email\.email \?\? ""/);
...
test("admin add profile aligns email persistence on repair path", () => {
  ...
  assert.match(adminCreateSource, /profileVerificationPayloadForInsert/);
