...
export function isSocialProfileSnapshotAccountCollectible(row: SupabaseRecord) {
  const lifecycle = readString(row.admin_lifecycle_status, "").toLowerCase();
  const status = readString(row.status, "").toLowerCase();
  const archived = new Set(["archived", "trashed", "deleted"]);
  const lifecycleEligible = lifecycle
    ? lifecycle === "active"
    : !archived.has(status);
  return lifecycleEligible
    && Boolean(readString(row.id, ""))
    && Boolean(normalizeSocialProfileUsername(row.username));
}
...
async function loadActiveSocialProfileAccounts(supabase: Supabase) {
  ...
  return ((accountsResult.data ?? []) as SupabaseRecord[])
    .filter(isSocialProfileSnapshotAccountCollectible);
}
...
export async function enqueueDailySocialProfileSnapshotJobs(input: {
  ...
  for (const account of active) {
    ...
    const observedAt = now.toISOString();
    planned += 1;
    // Organic profile growth is independent from the Worker scheduler.  The
    // daily cron therefore classifies every non-archived account even when it
    // is Manual, paused, inactive at scheduler level, or has no recent run.
    const trigger = "daily_fallback" as const;
    ...
    const localDate = businessDayKeyFromIso(observedAt, resolved.timezone);
    try {
      ...
      results.push({
        accountId,
        username,
        classification: "classification_failed" as const,
        reason: "guard_rpc_failed",
        created: false,
...
export async function processClaimedSocialProfileSnapshotJobs(input: {
  ...
  for (const [index, job] of jobs.entries()) {
    ...
    providerCalls += 1;
    const username = readString(job.username_normalized, "");
    let lookup: InstagramPublicProfileLookupResult;
    try {
      lookup = await (input.lookup ?? lookupInstagramPublicProfile)(username);
    } catch {
      lookup = {
        ok: false,
    ...
    let persisted: Awaited<ReturnType<typeof persistSocialProfileLookup>> | { ok: false; reason: string };
    try {
      persisted = lookup.status === "provider_error" && lookup.reason === "provider_exception"
        ? { ok: false as const, reason: "provider_exception" }
        : await (input.persist ?? persistSocialProfileLookup)({
      ...
      persisted = { ok: false as const, reason: "snapshot_persist_exception" };
    }
