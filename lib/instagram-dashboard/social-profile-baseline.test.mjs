...
test("shared processor marks the third transient failure as retry exhausted", async () => {
  const updates = [];
  const result = await processClaimedSocialProfileSnapshotJobs({
    jobs: [{ ...claimedJob(1), attempts: 3 }],
    maxProviderCalls: 1,
    supabase: processorSupabase(updates),
    lookup: async () => ({
      status: "unavailable",
    ...
    persist: async () => ({ ok: false, reason: "provider_invalid_response" }),
    pause: async () => undefined,
  });
  assert.equal(result.failedTerminal, 1);
  assert.equal(updates[0].values.status, "failed");
  assert.equal(updates[0].values.last_error_code, "retry_exhausted:provider_invalid_response");
});
...
function oneShotSources() {
  const source = readFileSync(new URL("./social-profile-baseline.ts", import.meta.url), "utf8");
  const route = readFileSync(new URL("../../app/api/instagram-dashboard/internal/social-profile-snapshots/baseline/route.ts", import.meta.url), "utf8");
  return { source, route, sources: `${source}\n${route}` };
}
