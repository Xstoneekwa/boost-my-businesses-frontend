...
test("operator review notifications deliver Slack and Discord with canonical delivery audit", async () => {
  ...
  assert.match(slack.text, /incident_id=incident-1&action_id=action-1\|Open Incidents\/Actions>/);
  assert.match(slack.blocks[1].text.text, /Open Incidents\/Actions/);
  assert.match(discord.content, /\[Open Incidents\/Actions\]\(https:\/\/www\.boostmybusinesses\.com\/instagram-dashboard\/incidents\?incident_id=incident-1&action_id=action-1\)/);
  assert.equal(operatorReviewActionUrl(input), "https://www.boostmybusinesses.com/instagram-dashboard/incidents?incident_id=incident-1&action_id=action-1");
