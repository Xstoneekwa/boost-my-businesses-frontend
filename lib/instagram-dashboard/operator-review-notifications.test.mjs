...
import { deliverOperatorReviewNotifications, operatorReviewActionUrl } from "./operator-review-notifications.ts";
...
test("operator review notifications deliver Slack and Discord with canonical delivery audit", async () => {
  ...
  const slack = posted.find((entry) => entry.channel === "slack").body;
  const discord = posted.find((entry) => entry.channel === "discord").body;
  assert.match(slack.text, /<https:\/\/www\.boostmybusinesses\.com\/instagram-dashboard\/incidents\?incident_id=incident-1\|Open Incidents\/Actions>/);
  assert.match(slack.blocks[1].text.text, /Open Incidents\/Actions/);
  assert.match(discord.content, /\[Open Incidents\/Actions\]\(https:\/\/www\.boostmybusinesses\.com\/instagram-dashboard\/incidents\?incident_id=incident-1\)/);
  assert.equal(operatorReviewActionUrl(input), "https://www.boostmybusinesses.com/instagram-dashboard/incidents?incident_id=incident-1");
...
test("review endpoint uses dedicated RPC and never updates dashboard actions directly", () => {
  ...
  const compositeFix = readFileSync(new URL("../../supabase/migrations/20260714161000_fix_operator_review_composite_return.sql", import.meta.url), "utf8");
  ...
  assert.match(compositeFix, /select transitioned\.\*/);
  assert.match(compositeFix, /from public\.transition_account_dashboard_action\(/);
  assert.doesNotMatch(compositeFix, /select public\.transition_account_dashboard_action\(/);
