...
describe("header payment method resolution", () => {
  it("shows subscription default for a standard single-account tenant", () => {
    const header = resolveHeaderPaymentMethod({
      ...
      agencyCustomerLevelShared: false,
      accountRows: [{
        accountId: "acct-a",
        username: "brand",
        planLabel: "Pro",
  ...
  it("marks agency header as shared when every account uses the same customer-level card", () => {
    const shared = {
      available: true,
      brand: "Visa",
      last4: "4242",
    ...
    const header = resolveHeaderPaymentMethod({
      ...
      agencyCustomerLevelShared: true,
      accountRows: [
  ...
  it("does not invent agency default when subscriptions only share a non-customer card", () => {
    const shared = {
      available: true,
      brand: "Visa",
      last4: "4242",
    ...
    assert.equal(header.scope, "none");
  });
});
