...
describe("stripe test checkout foundation v1", () => {
  ...
  it("prepares authorized public Growth monthly Stripe checkout without preprovisioning", async () => {
    ...
    assert.equal(stripe.calls.customersCreate.length, 0);
    assert.equal(stripe.calls.checkoutCreate[0].payload.customer_email, "client@example.com");
    assert.equal(stripe.calls.checkoutCreate[0].payload.line_items[0].price, "price_boostaigrowth1");
    assert.doesNotMatch(JSON.stringify(stripe.calls.checkoutCreate[0].payload), /price_from_browser|browser-client-id|password|payment_method_types/);
  });
...
describe("stripe test webhook foundation v1", () => {
  ...
  it("paid public checkout webhook provisions once without phone/runtime side effects", async () => {
    ...
    const fulfilled = await handleStripeWebhookEvent(supabase, paidEvent);
    const duplicate = await handleStripeWebhookEvent(supabase, paidEvent);
    setStripeClientForTests(null);
