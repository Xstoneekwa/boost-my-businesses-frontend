...
import { resolveExistingAccountCheckoutEmail } from "./stripe-existing-account-binding.ts";
import {
...
describe("stripe test checkout foundation v1", () => {
  it("resolves an existing-account checkout email from canonical client metadata", async () => {
    const db = createFakeSupabase({
      clients: [{ id: "client-1", metadata: { contact_email: "Owner@Example.com" } }],
    });
    const result = await resolveExistingAccountCheckoutEmail(db.supabase, "client-1");
  ...
  it("creates a Stripe Test contract for an existing account with immutable canonical binding metadata", async () => {
    const accountId = "account-existing-1";
    const db = createFakeSupabase({
      client_instagram_accounts: [{ client_id: "client-1", account_id: accountId }],
      ig_accounts: [{ id: accountId, status: "active", admin_lifecycle_status: "active" }],
      account_package_summary: [{ account_id: accountId, commercial_package_code: "pro" }],
    });
    const stripe = createFakeStripe();
    const result = await createStripeSubscriptionCheckoutSession(db.supabase, {
      commercialMode: "full_cycle",
      ...
      purchaserEmail: "client@example.com",
      flowType: "additional_account",
      idempotencyKey: "existing-account-modern-contract",
      clientId: "client-1",
      targetAccountId: accountId,
      billingSource: "stripe_test_modern",
      commercialMigrationReason: "lifecycle_e2e_certification",
      successUrl: "https://app.example.test/commercial/stripe-test/success?session_id={CHECKOUT_SESSION_ID}",
      cancelUrl: "https://app.example.test/commercial/stripe-test/cancel",
      allowedOrigins: ["https://app.example.test"],
      stripe,
    ...
    assert.equal(result.ok, true, JSON.stringify(result));
    assert.equal(db.tables.commercial_stripe_checkout_attempts[0].account_id, accountId);
    assert.equal(db.tables.commercial_checkout_sessions[0].metadata.target_account_id, accountId);
    assert.equal(db.tables.commercial_checkout_sessions[0].metadata.billing_source, "stripe_test_modern");
    assert.equal(db.tables.commercial_checkout_sessions[0].metadata.commercial_migration_reason, "lifecycle_e2e_certification");
    assert.equal(stripe.calls.checkoutCreate[0].payload.metadata.target_account_id, accountId);
    assert.equal(stripe.calls.checkoutCreate[0].payload.subscription_data.metadata.billing_source, "stripe_test_modern");
    assert.equal(stripe.calls.customersCreate[0].payload.metadata.client_id, "client-1");
  });

  it("rejects an existing-account package mismatch before Stripe customer or subscription creation", async () => {
    const accountId = "account-existing-package-mismatch";
    const db = createFakeSupabase({
      client_instagram_accounts: [{ client_id: "client-1", account_id: accountId }],
      ig_accounts: [{ id: accountId, status: "active", admin_lifecycle_status: "active" }],
      account_package_summary: [{ account_id: accountId, commercial_package_code: "growth" }],
    });
    const stripe = createFakeStripe();
    const result = await createStripeSubscriptionCheckoutSession(db.supabase, {
      commercialMode: "full_cycle",
      ...
      purchaserEmail: "client@example.com",
      flowType: "additional_account",
      idempotencyKey: "existing-account-package-mismatch",
      clientId: "client-1",
      targetAccountId: accountId,
      successUrl: "https://app.example.test/commercial/stripe-test/success?session_id={CHECKOUT_SESSION_ID}",
      cancelUrl: "https://app.example.test/commercial/stripe-test/cancel",
    ...
    assert.equal(result.ok, false);
    assert.equal(result.code, "target_account_package_mismatch");
    assert.equal(stripe.calls.customersCreate.length, 0);
    assert.equal(stripe.calls.checkoutCreate.length, 0);
    assert.equal(db.tables.commercial_checkout_sessions.length, 0);
  });
  ...
  it("prepares authorized public Growth monthly Stripe checkout without preprovisioning", async () => {
    const { result, stripe, tables, authUsers } = await publicCheckout();
    assert.equal(result.ok, true, JSON.stringify(result));
...
describe("stripe test webhook foundation v1", () => {
  ...
  it("paid existing-account checkout binds the exact entitlement and preserves legacy history", async () => {
    const accountId = "account-existing-paid";
    const legacySubscription = {
      id: "legacy-subscription-existing-paid",
      client_id: "client-1",
      status: "active",
      package_code: "pro",
    };
    const legacyBinding = {
      id: "legacy-binding-existing-paid",
      subscription_id: legacySubscription.id,
      account_id: accountId,
    };
    const db = createFakeSupabase({
      clients: [{ id: "client-1", tenant_id: "tenant-1", status: "active" }],
      tenant_users: [{ user_id: "admin-1", tenant_id: "client-1", role: "superadmin" }],
      client_users: [{ id: "client-user-1", client_id: "client-1", auth_user_id: "admin-1", role: "owner", status: "active" }],
      client_instagram_accounts: [{ client_id: "client-1", account_id: accountId }],
      ig_accounts: [{ id: accountId, status: "active", admin_lifecycle_status: "active" }],
      account_package_summary: [{ account_id: accountId, commercial_package_code: "pro" }],
      client_subscriptions: [{ ...legacySubscription }],
      client_subscription_accounts: [{ ...legacyBinding }],
    });
    await db.supabase.auth.admin.createUser({ email: "admin@example.com", password: "not-a-real-secret" });
    db.authUsers[0].id = "admin-1";
    const stripe = createFakeStripe();
    const checkoutResult = await createStripeSubscriptionCheckoutSession(db.supabase, {
      commercialMode: "full_cycle",
      packageKey: "pro",
      planKey: "pro",
      billingIntervalMonths: 1,
      ...
      purchaserEmail: "client@example.com",
      flowType: "additional_account",
      idempotencyKey: "existing-account-paid-contract",
      clientId: "client-1",
      authUserId: "admin-1",
      targetAccountId: accountId,
      billingSource: "stripe_test_modern",
    ...
    assert.equal(checkoutResult.ok, true, JSON.stringify(checkoutResult));

    const attempt = db.tables.commercial_stripe_checkout_attempts[0];
    setStripeClientForTests(stripe);
    const fulfilled = await handleStripeWebhookEvent(db.supabase, event("checkout.session.completed", {
      id: attempt.stripe_checkout_session_id,
      livemode: false,
      mode: "subscription",
      payment_status: "paid",
    ...
    assert.equal(fulfilled.ok, true, JSON.stringify(fulfilled));
    assert.equal(db.tables.client_account_entitlements.length, 1);
    assert.equal(db.tables.client_account_entitlements[0].status, "entitlement_consumed");
    assert.equal(db.tables.client_account_entitlements[0].account_id, accountId);
    assert.equal(attempt.status, "fulfilled");
    assert.equal(attempt.account_id, accountId);
    assert.equal(attempt.client_account_entitlement_id, db.tables.client_account_entitlements[0].id);
    assert.equal(db.tables.commercial_stripe_subscriptions.length, 1);
    assert.equal(db.tables.commercial_stripe_subscriptions[0].account_id, accountId);
    assert.equal(
      db.tables.commercial_stripe_subscriptions[0].client_account_entitlement_id,
      db.tables.client_account_entitlements[0].id,
    );
    assert.equal(db.tables.commercial_checkout_sessions[0].metadata.billing_source, "stripe_test_modern");
    assert.equal(
      db.tables.commercial_checkout_sessions[0].metadata.commercial_migration_reason,
      "lifecycle_e2e_certification",
    );
    assert.deepEqual(db.tables.client_subscriptions, [legacySubscription]);
    assert.deepEqual(db.tables.client_subscription_accounts, [legacyBinding]);
  });
