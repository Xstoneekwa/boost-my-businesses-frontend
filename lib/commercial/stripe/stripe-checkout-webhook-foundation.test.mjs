...
describe("stripe checkout webhook migration invariants", () => {
  it("does not allow null package fields unless the row is explicitly outreach_only", () => {
    ...
    assert.match(migration, /\(commercial_mode is null or commercial_mode = 'full_cycle'\)[\s\S]*plan_key in \('growth', 'pro', 'premium'\)/);
    assert.match(migration, /metadata->>'commercial_mode'\) is null or metadata->>'commercial_mode' = 'full_cycle'[\s\S]*commercial_package_code in \('growth', 'pro', 'premium'\)/);
    assert.match(migration, /commercial_mode = 'outreach_only'[\s\S]*plan_key is null/);
    assert.match(migration, /metadata->>'commercial_mode' = 'outreach_only'[\s\S]*commercial_package_code is null/);
  });
});
