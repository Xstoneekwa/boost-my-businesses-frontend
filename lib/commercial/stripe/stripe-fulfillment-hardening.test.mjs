...
describe("plan change checkout creation guards", () => {
  ...
  it("fulfillment requires sync before internal plan change activation", () => {
    ...
    const activateIndex = planChangeBlock.indexOf("activatePlanChangeQuote");
    assert.ok(syncIndex >= 0 && activateIndex > syncIndex);
...
describe("session-status ownership and public preservation", () => {
  ...
  it("success page remains informational only", () => {
    const source = readFileSync(new URL("../../../app/commercial/stripe-test/success/page.tsx", import.meta.url), "utf8");
    assert.match(source, /informational only/i);
    assert.match(source, /session-status/);
    assert.doesNotMatch(source, /activateClientAccountEntitlementFromCheckout/);
  });
