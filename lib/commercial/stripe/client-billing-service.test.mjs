  ...
  return {
    ...
    async listSubscriptions() {
      return input.subscriptions ?? [{
        ...
        status: "active",
        current_period_end: "2027-07-20T13:44:51.000Z",
      }];
    },
    async listInvoices() {
      return invoices;
...
describe("canonical billing date", () => {
  it("reads the current Stripe API period end from subscription items", () => {
    assert.equal(stripeSubscriptionPeriodEnd({
      current_period_end: null,
      items: { data: [{ current_period_end: 1816091091 }] },
    }), "2027-07-20T13:44:51.000Z");
  });

  it("keeps card expiry independent from next payment date", async () => {
    ...
    const view = await buildClientBillingView({
      ...
      packageSummaries: new Map([packageSummary("acct-a", "Growth", "growth")]),
    });
    assert.equal(view.accounts[0].billingDate.dateIso, "2027-07-20T13:44:51.000Z");
    assert.equal(view.accounts[0].billingDate.label, "Prochain prélèvement");
    assert.equal(view.accounts[0].paymentMethod.expMonth, 8);
    assert.equal(view.accounts[0].paymentMethod.expYear, 2029);
  });
});
