...
function validMessage(overrides = {}) {
  return {
    subject: null,
    body: `Hi Glow Studio, I noticed you're based in Johannesburg. Your next customers are already on Instagram. BMB identifies relevant Instagram audiences around similar businesses and uses targeted interactions to bring those people to your profile, helping grow your visibility among potential customers. ${quality.AUDIENCE_CTA}`,
    channel: "instagram",
    angle: "A",
    template_version: "IG_BEAUTY_ANGLE_A_V1",
    personalization_summary: "Used the verified business name and city.",
    personalization_evidence: { key: "city", quote: "Johannesburg" },
...
test("generation uses server greeting, locked offer, previous guard feedback and prompt V3", async () => {
  let requestBody;
  ...
  assert.equal(payload.prompt_version, "commercial_outreach_message_quality_v3");
...
test("processor never completes an invalid preview as ready; retry feedback is bounded by existing claim RPC", async () => {
  ...
  const processor = await moduleFromTs(`import { validateCommercialOutreachMessage, commercialOutreachContentHash } from ${JSON.stringify(validationUrl)};\nconst commercialOutreachCopyTemplateKey = () => "IG_BEAUTY_ANGLE_A_V3"; const COMMERCIAL_OUTREACH_PROMPT_VERSION = "commercial_outreach_message_quality_v3"; const buildCommercialOutreachFactLedger = () => ${JSON.stringify(facts)};\n${source}`);
  let attempted = 0; const completions = []; const prior = [];
  const fakeDb = {
    rpc: async (name, args) => {
      if (name.startsWith("claim_")) return { data: attempted < 2 ? [{ id: "item", lead_id: "lead", channel: "instagram", angle: "A", template_key: "IG_BEAUTY_ANGLE_A_V1", template_version: "IG_BEAUTY_ANGLE_A_V3", generation_attempt_count: attempted + 1, validation_codes: [] }] : [] };
  ...
  assert.equal(completions[1].p_payload.prompt_version, "commercial_outreach_message_quality_v3");
  assert.deepEqual(completions[1].p_validation_codes, []);
});

test("approval and edit run the real V3 guard before the existing owner-authorized mutation RPC", async () => {
