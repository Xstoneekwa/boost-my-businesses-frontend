...
function createLifecycleFakeSupabase(options = {}) {
  const tables = {
    ...
    commercial_account_lifecycle_operations: [],
    ...
    commercial_stripe_subscriptions: (options.accounts ?? [ACCOUNT_A]).map((accountId) => {
      const entitlementId = accountId === ACCOUNT_A ? ENTITLEMENT_A : ENTITLEMENT_B;
...
describe("commercial account lifecycle service", () => {
  it("pause active account with running runtime quiesces, pauses Stripe, keeps slot", async () => {
    ...
    assert.equal(result.stripeBillingPaused, true);
    assert.ok(result.pauseExpiresAt);
    assert.equal(result.runtimeQuiesced, true);
    ...
    assert.equal(tables.commercial_stripe_subscriptions[0].pause_collection_behavior, "void");
    assert.equal(tables.ig_runs[0].status, "stopped");
  });
