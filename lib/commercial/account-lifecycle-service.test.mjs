...
function entitlementRow(accountId, entitlementId) {
  return {
    id: entitlementId,
    client_id: CLIENT_ID,
...
function stripeProjection(entitlementId, subId, status = "active", accountId = ACCOUNT_A) {
  return {
...
function createLifecycleFakeSupabase(options = {}) {
  const tables = {
    ...
    commercial_stripe_subscriptions: (options.omitDefaultStripeSubscriptions ? [] : (options.accounts ?? [ACCOUNT_A]).map((accountId) => {
      const entitlementId = accountId === ACCOUNT_A ? ENTITLEMENT_A : ENTITLEMENT_B;
      return stripeProjection(entitlementId, accountId === ACCOUNT_A ? SUB_A : SUB_B, "active", accountId);
  ...
  function makeUpdateBuilder(table, patch) {
    ...
    const builder = {
      ...
      then(resolve, reject) {
        if (table === "client_email_send_intents" && options.emailIntentUpdateFails) {
          return Promise.resolve({ error: { message: "email_intent_update_failed" } }).then(resolve, reject);
        }
  ...
  const supabase = {
    from(table) {
      const query = {
        ...
        is(column, value) {
          query._filters.push({ column, value, op: "is" });
          return query;
        },
        order() { return query; },
        limit() { return query; },
    ...
    rpc(name, args) {
      ...
      if (name === "release_client_instagram_account_capacity_v1") {
        const link = tables.client_instagram_accounts.find((row) => row.account_id === args.p_account_id && row.active === true);
        if (!link) return Promise.resolve({ data: { ok: false, reason: "active_client_account_link_missing" }, error: null });
        if (link.capacity_status === "released_terminal") {
          return Promise.resolve({ data: { ok: true, status: "already_released_terminal" }, error: null });
        }
        const account = tables.ig_accounts.find((row) => row.id === args.p_account_id);
        const state = tables.commercial_account_lifecycle_states.find((row) => row.account_id === args.p_account_id);
        const operation = tables.commercial_account_lifecycle_operations.find((row) => row.id === args.p_operation_id);
        const entitlement = tables.client_account_entitlements.find((row) => row.id === state?.terminal_cancel_entitlement_id);
        const subscription = tables.commercial_stripe_subscriptions.find((row) => row.stripe_subscription_id === state?.terminal_cancel_stripe_subscription_id);
        const complete = account?.admin_lifecycle_status === "cancelled"
          && state?.commercial_state === "cancelled"
          && state?.terminal_cancel_operation_id === args.p_operation_id
          && operation?.operation_type === "cancel"
          && operation?.state === "completed"
          && entitlement?.status === "entitlement_cancelled"
          && ["canceled", "cancelled", "incomplete_expired"].includes(subscription?.status);
        if (!complete) return Promise.resolve({ data: { ok: false, reason: "terminal_cancellation_evidence_incomplete" }, error: null });
        Object.assign(link, {
          capacity_status: "released_terminal",
          capacity_released_at: new Date().toISOString(),
          capacity_release_reason: args.p_reason,
          capacity_release_operation_id: args.p_operation_id,
        });
        return Promise.resolve({ data: { ok: true, status: "released_terminal" }, error: null });
      }
      if (name === "record_commercial_cancel_terminal_v1") {
        const account = tables.ig_accounts.find((row) => row.id === args.p_account_id);
        const state = tables.commercial_account_lifecycle_states.find((row) => row.account_id === args.p_account_id);
        const operation = tables.commercial_account_lifecycle_operations.find((row) => row.id === args.p_operation_id);
        const entitlement = tables.client_account_entitlements.find((row) => row.id === args.p_entitlement_id);
        const subscription = tables.commercial_stripe_subscriptions.find((row) => row.stripe_subscription_id === args.p_stripe_subscription_id);
        if (state?.terminal_cancel_operation_id) {
          const same = state.terminal_cancel_operation_id === args.p_operation_id
            && state.terminal_cancel_entitlement_id === args.p_entitlement_id
            && state.terminal_cancel_stripe_subscription_id === args.p_stripe_subscription_id;
          return Promise.resolve({ data: same
            ? { ok: true, status: "already_recorded" }
            : { ok: false, reason: "terminal_cancel_provenance_conflict" }, error: null });
        }
        const complete = account?.admin_lifecycle_status === "cancelled"
          && state?.commercial_state === "cancelled"
          && operation?.operation_type === "cancel"
          && ["in_progress", "completed"].includes(operation?.state)
          && entitlement?.account_id === args.p_account_id
          && entitlement?.status === "entitlement_cancelled"
          && subscription?.account_id === args.p_account_id
          && subscription?.client_account_entitlement_id === args.p_entitlement_id
          && ["canceled", "cancelled", "incomplete_expired"].includes(subscription?.status);
        if (!complete) return Promise.resolve({ data: { ok: false, reason: "terminal_cancellation_evidence_incomplete" }, error: null });
        operation.state = "completed";
        Object.assign(state, {
          terminal_cancel_operation_id: args.p_operation_id,
          terminal_cancelled_at: new Date().toISOString(),
          terminal_cancel_entitlement_id: args.p_entitlement_id,
          terminal_cancel_stripe_subscription_id: args.p_stripe_subscription_id,
        });
        return Promise.resolve({ data: { ok: true, status: "recorded" }, error: null });
      }
...
describe("commercial account lifecycle service", () => {
  ...
  it("pause reconciles an expired email claim but preserves a live claim as a blocker", async () => {
    const expiredAt = new Date(Date.now() - 60_000).toISOString();
    const liveUntil = new Date(Date.now() + 60_000).toISOString();
    const { supabase, tables } = createLifecycleFakeSupabase({
      accounts: [ACCOUNT_A, ACCOUNT_B],
      emailJobs: [
        { id: "email-expired", account_id: ACCOUNT_A, status: "claimed", claim_token: "11111111-1111-4111-8111-111111111111", claimed_at: expiredAt, claim_expires_at: expiredAt },
        { id: "email-live", account_id: ACCOUNT_B, status: "claimed", claim_token: "22222222-2222-4222-8222-222222222222", claimed_at: new Date().toISOString(), claim_expires_at: liveUntil },
      ],
    });
    const stripe = createStripeGateway();

    const expiredResult = await executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "pause",
      idempotencyKey: "pause-expired-email-claim",
      reason: "manual_pause",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    assert.equal(expiredResult.ok, true);
    const expiredIntent = tables.client_email_send_intents.find((row) => row.id === "email-expired");
    assert.equal(expiredIntent.status, "canceled");
    assert.equal(expiredIntent.dispatch_last_error_code, "lifecycle_quiesce_expired_claim");
    assert.equal(expiredIntent.claim_token, null);
    assert.equal(expiredIntent.claimed_at, null);
    assert.equal(expiredIntent.claim_expires_at, null);
    assert.equal(Object.hasOwn(expiredIntent, "last_error"), false);
    assert.equal(Object.hasOwn(expiredIntent, "updated_at"), false);
    assert.equal(tables.client_email_send_intents.find((row) => row.id === "email-live").status, "claimed");

    const liveResult = await executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_B,
      operationType: "pause",
      idempotencyKey: "pause-live-email-claim",
      reason: "manual_pause",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    assert.equal(liveResult.ok, false);
    assert.equal(liveResult.actionRequiredReason, "runtime_still_active");
    assert.equal(tables.ig_accounts.find((row) => row.id === ACCOUNT_B).admin_lifecycle_status, "active");
  });

  it("pause restores the pre-pause lifecycle when email claim reconciliation fails", async () => {
    const expiredAt = new Date(Date.now() - 60_000).toISOString();
    const { supabase, tables } = createLifecycleFakeSupabase({
      emailIntentUpdateFails: true,
      emailJobs: [{
        id: "email-expired",
        account_id: ACCOUNT_A,
        status: "claimed",
    ...
    await assert.rejects(
      executeCommercialAccountLifecycle({
        ...
        accountId: ACCOUNT_A,
        operationType: "pause",
        idempotencyKey: "pause-email-reconciliation-fails",
        reason: "manual_pause",
        actor: ACTOR,
        stripeGateway: createStripeGateway(),
      }),
      /email_intent_update_failed/,
    );

    assert.equal(tables.ig_accounts[0].admin_lifecycle_status, "active");
    assert.equal(tables.commercial_account_lifecycle_states[0].commercial_state, "active");
    assert.equal(tables.commercial_account_lifecycle_states[0].stripe_billing_paused, false);
  });
  ...
  it("cancel from paused applies full teardown", async () => {
    ...
    assert.equal(stripe.calls[0].op, "cancel");
    assert.equal(tables.client_account_entitlements[0].status, "entitlement_cancelled");
    assert.equal(tables.client_instagram_accounts[0].active, true, "ownership/history visibility is preserved");
    assert.equal(tables.client_instagram_accounts[0].capacity_status, "released_terminal");
    assert.equal(tables.client_instagram_accounts[0].capacity_release_reason, "commercial_cancel_completed");
    assert.equal(typeof tables.client_instagram_accounts[0].capacity_release_operation_id, "string");
  });

  it("two simultaneous cancel requests converge on one terminal operation and one Stripe write", async () => {
    const { supabase, tables } = createLifecycleFakeSupabase();
    let releaseStripe;
    let stripeEntered;
    const stripeEnteredPromise = new Promise((resolve) => { stripeEntered = resolve; });
    const releaseStripePromise = new Promise((resolve) => { releaseStripe = resolve; });
    const stripe = createStripeGateway();
    const originalCancel = stripe.cancelSubscriptionImmediately.bind(stripe);
    stripe.cancelSubscriptionImmediately = async (...args) => {
      await originalCancel(...args);
      stripeEntered();
      await releaseStripePromise;
    };

    const first = executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-concurrent-first",
      reason: "manual_cancel",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    await stripeEnteredPromise;

    const second = executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-concurrent-second",
      reason: "manual_cancel",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    ...
    assert.equal(secondResult.converged, true);
    assert.equal(stripe.calls.filter((call) => call.op === "cancel").length, 1);
    assert.equal(tables.commercial_account_lifecycle_operations.length, 1);
    assert.equal(tables.commercial_account_lifecycle_states[0].terminal_cancel_operation_id, firstResult.operationId);
    assert.equal(tables.client_instagram_accounts[0].capacity_status, "released_terminal");
  });

  it("terminal cancellation reload remains cancelled and idempotently heals capacity", async () => {
    const terminalOperationId = "cancel-terminal-old";
    const laterFailedOperationId = "cancel-later-failed";
    const { supabase, tables } = createLifecycleFakeSupabase({
      ...
      lifecycleStates: [{
        ...
        entitlement_id: ENTITLEMENT_A,
        stripe_subscription_id: SUB_A,
        last_operation_id: laterFailedOperationId,
        terminal_cancel_operation_id: terminalOperationId,
        terminal_cancelled_at: "2026-08-20T12:00:00.000Z",
        terminal_cancel_entitlement_id: ENTITLEMENT_A,
        terminal_cancel_stripe_subscription_id: SUB_A,
      }],
      lifecycleOperations: [{
        ...
        id: laterFailedOperationId,
        account_id: ACCOUNT_A,
        operation_type: "resume",
        state: "failed",
      }],
      omitDefaultStripeSubscriptions: true,
      extraStripeSubscriptions: [stripeProjection(ENTITLEMENT_A, SUB_A, "canceled", ACCOUNT_A)],
    });
    tables.client_account_entitlements[0].status = "entitlement_cancelled";
    const stripe = createStripeGateway();
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-terminal-reload",
      reason: "terminal_reload_reconciliation",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    ...
    assert.equal(result.capacityReleaseStatus, "released");
    assert.equal(stripe.calls.length, 0);
    assert.equal(tables.client_instagram_accounts[0].active, true);
    assert.equal(tables.client_instagram_accounts[0].capacity_status, "released_terminal");
    assert.equal(tables.client_instagram_accounts[0].capacity_release_operation_id, terminalOperationId);

    const firstReleasedAt = tables.client_instagram_accounts[0].capacity_released_at;
    const replay = await executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-terminal-reload-second",
      reason: "terminal_reload_reconciliation",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    assert.equal(replay.ok, true);
    assert.equal(replay.capacityReleaseStatus, "released");
    assert.equal(tables.client_instagram_accounts[0].capacity_released_at, firstReleasedAt);
    assert.equal(tables.client_instagram_accounts[0].capacity_release_operation_id, terminalOperationId);
  });

  it("partial terminal evidence stays occupied and fails closed", async () => {
    const { supabase, tables } = createLifecycleFakeSupabase({
      adminStatus: { [ACCOUNT_A]: "cancelled" },
      lifecycleStates: [{
        account_id: ACCOUNT_A,
        ...
        entitlement_id: ENTITLEMENT_A,
        stripe_subscription_id: SUB_A,
        last_operation_id: "later-observability-only-operation",
      }],
      omitDefaultStripeSubscriptions: true,
      extraStripeSubscriptions: [stripeProjection(ENTITLEMENT_A, SUB_A, "canceled", ACCOUNT_A)],
    });
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-terminal-incomplete",
      reason: "terminal_reload_reconciliation",
      actor: ACTOR,
      stripeGateway: createStripeGateway(),
    });
    ...
    assert.equal(result.ok, false);
    assert.equal(result.actionRequiredReason, "terminal_cancellation_incomplete");
    assert.equal(tables.client_instagram_accounts[0].capacity_status, "occupied");
    assert.equal(tables.client_instagram_accounts[0].active, true);
  });
  ...
  it("supports repeated pause, resume, and pause without stale lifecycle state", async () => {
    const { supabase, tables } = createLifecycleFakeSupabase();
    const stripe = createStripeGateway();

    const firstPause = await executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "pause",
      idempotencyKey: "cycle-pause-1",
      reason: "manual_pause",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    ...
    const resume = await executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "resume",
      idempotencyKey: "cycle-resume-1",
      reason: "manual_resume",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    ...
    const secondPause = await executeCommercialAccountLifecycle({
      supabase,
      accountId: ACCOUNT_A,
      operationType: "pause",
      idempotencyKey: "cycle-pause-2",
      reason: "manual_pause",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    assert.equal(secondPause.ok, true);
    assert.deepEqual(stripe.calls.map((call) => call.op), ["pause", "resume", "pause"]);
    assert.equal(tables.ig_accounts[0].admin_lifecycle_status, "paused");
    assert.equal(tables.commercial_account_lifecycle_states[0].commercial_state, "paused");
    assert.equal(tables.commercial_stripe_subscriptions[0].billing_paused, true);
  });
