...
function createLifecycleFakeSupabase(options = {}) {
  const tables = {
    ...
    client_email_lifecycle_episodes: [...(options.emailLifecycleEpisodes ?? [])],
    auto_restart_decisions: [...(options.autoRestartJobs ?? [])],
    runtime_incidents: [...(options.runtimeIncidents ?? [])],
    client_account_dashboard_actions: [...(options.dashboardActions ?? [])],
    ig_account_identity_state: [...(options.identityStates ?? [])],
    ig_targets: [...(options.targets ?? [])],
  ...
  const supabase = {
    ...
    rpc(name, args) {
      ...
      if (name === "terminalize_client_email_lifecycle_episode_v1") {
        if (options.emailEpisodeTerminalizationFails) {
          return Promise.resolve({ data: null, error: { message: "pause_email_episode_terminalization_failed" } });
        }
        const episode = tables.client_email_lifecycle_episodes.find((row) =>
          row.id === args.p_parent_id
          && row.account_id === args.p_account_id
          && row.client_id === args.p_client_id
          && row.category === args.p_category
          && row.episode_key === args.p_parent_episode_key
        );
        if (!episode) {
          return Promise.resolve({ data: { ok: false, code: "lifecycle_episode_not_found" }, error: null });
        }
        const changed = episode.status === "active";
        if (changed) {
          episode.status = "resolved";
          episode.resolved_at = new Date().toISOString();
          episode.close_reason = "account_reactivated";
          tables.client_email_send_intents.forEach((intent) => {
            if (intent.lifecycle_episode_id === episode.id && ["pending", "scheduled", "claimed"].includes(intent.status)) {
              intent.status = "canceled";
              intent.dispatch_last_error_code = "lifecycle_episode_terminalized";
            }
          });
        }
        return Promise.resolve({
          data: { ok: true, terminal_status: episode.status, changed, idempotent: !changed },
          error: null,
        });
      }
...
describe("commercial account lifecycle service", () => {
  ...
  it("pause without active run still pauses Stripe and keeps slot", async () => {
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      actor: ACTOR,
      stripeGateway: stripe,
      lifecycleEmailHandoff: async (input) => {
        handoffs.push(input);
        return {
          scope: { accountId: input.accountId, category: input.category },
          materialize: { candidates: 2, materialized: 2, skipped: 0, failed: 0 },
          dispatch: { candidates: 1, submitted: 1 },
        };
      },
    });
    ...
    assert.equal(handoffs[0].accountId, ACCOUNT_A);
    assert.equal(handoffs[0].category, "account_paused");
  });

  it("keeps a successful Pause committed when immediate email handoff defers to cron", async () => {
    const { supabase, tables } = createLifecycleFakeSupabase();
    const stripe = createStripeGateway();
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "pause",
      idempotencyKey: "pause-email-deferred",
      reason: "manual_pause",
      actor: ACTOR,
      stripeGateway: stripe,
      lifecycleEmailHandoff: async () => {
    ...
    const audit = tables.ig_action_logs.find(
      (row) => row.message === "commercial_pause_email_handoff_deferred",
    );
    assert.ok(audit);
    assert.equal(audit.payload.recovery, "periodic_cron");
    assert.equal(audit.payload.reason, "provider_temporarily_unavailable");
  });
  ...
  it("resume synchronously resolves active pause episodes and preserves sent history", async () => {
    const pauseExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const { supabase, tables } = createLifecycleFakeSupabase({
      adminStatus: { [ACCOUNT_A]: "paused" },
      lifecycleStates: [{
      ...
      emailLifecycleEpisodes: [{
        id: "pause-episode-a",
        account_id: ACCOUNT_A,
        client_id: CLIENT_ID,
        category: "account_paused",
        episode_key: "pause-episode-key-a",
        status: "active",
      }],
      emailJobs: [{
        id: "sent-pause-email-a",
        account_id: ACCOUNT_A,
        lifecycle_episode_id: "pause-episode-a",
        status: "sent",
      }],
    });
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "resume",
      idempotencyKey: "resume-terminalizes-pause-episode",
      reason: "manual_resume",
      actor: ACTOR,
      stripeGateway: createStripeGateway(),
    });
    ...
    assert.equal(result.ok, true);
    assert.equal(tables.client_email_lifecycle_episodes[0].status, "resolved");
    assert.equal(tables.client_email_lifecycle_episodes[0].close_reason, "account_reactivated");
    assert.equal(tables.client_email_send_intents[0].status, "sent");
  });

  it("resume cancels only unsent pause intents when terminalizing the episode", async () => {
    const pauseExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const { supabase, tables } = createLifecycleFakeSupabase({
      adminStatus: { [ACCOUNT_A]: "paused" },
      lifecycleStates: [{
      ...
      emailJobs: [
        { id: "pending-a", account_id: ACCOUNT_A, lifecycle_episode_id: "pause-episode-a", status: "pending" },
        { id: "uncertain-a", account_id: ACCOUNT_A, lifecycle_episode_id: "pause-episode-a", status: "dispatch_uncertain" },
      ],
    });
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "resume",
      idempotencyKey: "resume-cancels-unsent-pause-intent",
      reason: "manual_resume",
      actor: ACTOR,
      stripeGateway: createStripeGateway(),
    });
    ...
    assert.equal(result.ok, true);
    assert.equal(tables.client_email_send_intents.find((row) => row.id === "pending-a").status, "canceled");
    assert.equal(tables.client_email_send_intents.find((row) => row.id === "uncertain-a").status, "dispatch_uncertain");
  });

  it("resume compensates to paused when pause episode terminalization fails", async () => {
    const pauseExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
    const { supabase, tables } = createLifecycleFakeSupabase({
      adminStatus: { [ACCOUNT_A]: "paused" },
      lifecycleStates: [{
      ...
      emailEpisodeTerminalizationFails: true,
    });
    const stripe = createStripeGateway();
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "resume",
      idempotencyKey: "resume-terminalization-failure-compensates",
      reason: "manual_resume",
      actor: ACTOR,
      stripeGateway: stripe,
    });
    ...
    assert.equal(result.ok, false);
    assert.equal(result.commercialState, "paused");
    assert.equal(result.actionRequiredReason, "pause_email_episode_terminalization_failed");
  ...
  it("cancel from active stops runtime, cancels Stripe, closes entitlement, releases capacity", async () => {
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      lifecycleEmailHandoff: async (input) => {
        ...
        return {
          scope: { accountId: input.accountId, category: input.category },
          materialize: { candidates: 1, materialized: 1, skipped: 0, failed: 0 },
          dispatch: { candidates: 1, submitted: 1 },
        };
      },
    });
    ...
    assert.equal(handoffs.length, 1);
    assert.equal(handoffs[0].category, "account_canceled");
  });

  it("operational identity, incident, review, CT, and readiness states do not block canonical cancel", async () => {
    const { supabase, tables } = createLifecycleFakeSupabase({
      accountStatus: { [ACCOUNT_A]: "needs_assistance" },
      identityStates: [{ account_id: ACCOUNT_A, status: "required_unverified" }],
      runtimeIncidents: [{
        id: "incident-open",
        account_id: ACCOUNT_A,
        status: "open",
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-operational-blockers",
      reason: "manual_cancel",
      actor: ACTOR,
      stripeGateway: stripe,
      lifecycleEmailHandoff: async (input) => ({
    ...
    assert.equal(tables.runtime_incidents[0].status, "open");
    assert.equal(tables.client_account_dashboard_actions[0].status, "pending");
  });

  it("a concurrent commercial lifecycle operation still blocks cancel", async () => {
    const { supabase } = createLifecycleFakeSupabase({
      lifecycleOperations: [{
  ...
  it("cancel email failure preserves terminal cancel and defers recovery to the periodic cron", async () => {
    const { supabase, tables } = createLifecycleFakeSupabase();
    const stripe = createStripeGateway();
    ...
    const result = await executeCommercialAccountLifecycle({
      ...
      accountId: ACCOUNT_A,
      operationType: "cancel",
      idempotencyKey: "cancel-email-deferred",
      reason: "manual_cancel",
      actor: ACTOR,
      stripeGateway: stripe,
      lifecycleEmailHandoff: async () => {
    ...
    assert.equal(result.commercialState, "cancelled");
    assert.equal(tables.ig_accounts[0].admin_lifecycle_status, "cancelled");
    const audit = tables.ig_action_logs.find((row) => row.message === "commercial_cancel_email_handoff_deferred");
    assert.equal(audit?.payload?.recovery, "periodic_cron");
  });

  it("idempotent cancel replay does not duplicate Stripe cancellation or email handoff", async () => {
    const { supabase } = createLifecycleFakeSupabase();
    const stripe = createStripeGateway();
    let handoffCalls = 0;
    const input = {
      ...
      actor: ACTOR,
      stripeGateway: stripe,
      lifecycleEmailHandoff: async (handoffInput) => {
        handoffCalls += 1;
        return {
          scope: { accountId: handoffInput.accountId, category: handoffInput.category },
          materialize: { candidates: 1, materialized: 1, skipped: 0, failed: 0 },
          dispatch: { candidates: 1, submitted: 1 },
        };
      },
    ...
    const first = await executeCommercialAccountLifecycle(input);
  ...
  it("supports repeated pause, resume, and pause without stale lifecycle state", async () => {
    ...
    const stripe = createStripeGateway();
    const handoffs = [];
    const lifecycleEmailHandoff = async (input) => {
      handoffs.push(input);
      return {
        scope: { accountId: input.accountId, category: input.category },
        materialize: { candidates: 2, materialized: 2, skipped: 0, failed: 0 },
