...
describe("commercial account lifecycle service", () => {
  ...
  it("pause reconciles an expired email claim but preserves a live claim as a blocker", async () => {
    ...
    const { supabase, tables } = createLifecycleFakeSupabase({
      accounts: [ACCOUNT_A, ACCOUNT_B],
      emailJobs: [
        { id: "email-expired", account_id: ACCOUNT_A, status: "claimed", claim_expires_at: expiredAt },
        { id: "email-live", account_id: ACCOUNT_B, status: "claimed", claim_expires_at: liveUntil },
      ],
    });
    const stripe = createStripeGateway();
    ...
    assert.equal(tables.client_email_send_intents.find((row) => row.id === "email-expired").status, "canceled");
