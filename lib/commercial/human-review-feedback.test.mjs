...
test("generation exceptions persist as failures and remain dry-run only", async () => {
  ...
  source = `const commercialOutreachCopyTemplateKey=()=>"IG_BEAUTY_ANGLE_A_V3"; const COMMERCIAL_OUTREACH_PROMPT_VERSION="fixture"; const buildCommercialOutreachFactLedger=()=>[{key:"business_name"},{key:"city"}];\n` + source;
  ...
  const fakeDb = {
    ...
    async rpc(name, args) { calls.push({ name, args }); return name === "claim_commercial_outreach_items_v1" ? { data: [{ id: "item", lead_id: "1", template_version: "IG_BEAUTY_ANGLE_A_V3" }] } : { data: {} }; },
  };
