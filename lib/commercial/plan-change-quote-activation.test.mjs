...
const ACCOUNT_ID = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa01";
      ...
      const api = {
        ...
        maybeSingle: async () => {
          if (table === "client_instagram_accounts") {
            return { data: { account_id: ACCOUNT_ID, client_id: CLIENT_ID }, error: null };
          }
          if (table === "ig_accounts") {
            return { data: { id: ACCOUNT_ID, status: "active", admin_lifecycle_status: "active" }, error: null };
          }
          if (table === "client_account_entitlements") {
        ...
        return {
          select() {
            const entitlementChain = {
              eq() { return entitlementChain; },
              order() { return entitlementChain; },
              limit() {
                return Promise.resolve({ data: [entitlement], error: null });
              },
...
test("createPlanChangeQuote persists postgres source_revision and activate succeeds unchanged", async () => {
  ...
  assert.equal(stored.change_scope, "per_account");
  assert.equal(stored.account_id, ACCOUNT_ID);
  assert.equal(supabase._rpcCalls.some((call) => call.name === "commercial_plan_change_source_revision_for_account_source"), true);
